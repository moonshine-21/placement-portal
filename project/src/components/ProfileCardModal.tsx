...
export function ProfileCardModal({ userId, onClose, onMessage, onCall }: Props) {
  ...
  return (
    ...
    >
      <div
        className="profile-popover w-full max-w-md max-h-[90vh] overflow-y-auto overflow-x-hidden scroll-thin animate-fade-in-scale"
      ...
      >
        {/* Banner — same treatment as the "view my profile" popover in the
            ...
            accent-color gradient. Taller than before to give the
            overlapping avatar room to sit low-left, Discord-style. */}
        <div className="profile-popover-banner relative h-32 w-full overflow-hidden rounded-t-[18px]">
        ...
        {loading ? (
          ...
          <>
            {/* Avatar overlapping the banner, bottom-left — the same
                ...
                left-to-right instead of the old centered stack. */}
            <div className="-mt-10 flex items-end gap-4">
              {profile.avatar_url ? (
                <img
                  src={profile.avatar_url}
                  alt=""
                  className="profile-popover-avatar h-24 w-24 flex-shrink-0 rounded-2xl border-4 border-[var(--bg-elevated)] object-cover"
                />
              ) : (
                <div className="profile-popover-avatar flex h-24 w-24 flex-shrink-0 items-center justify-center rounded-2xl border-4 border-[var(--bg-elevated)] bg-gradient-to-br from-[var(--accent)] to-[var(--accent-2)] text-2xl font-bold text-white">
                  {(profile.full_name || 'U').slice(0, 2).toUpperCase()}
                </div>
              )}
              <div className="min-w-0 pb-1">
                <p className="flex items-center gap-1.5 truncate text-lg font-bold text-[var(--text-primary)]">
                  {profile.full_name || 'Unnamed Student'}
                  {(profile.role === 'admin' || profile.role === 'owner') && <AdminBadge role={profile.role} />}
                </p>
                {profile.branch && (
                  <p className="mt-0.5 flex items-center gap-1 text-xs font-medium text-[var(--accent)]">
            ...
            {profile.bio && (
              <p className="mt-4 text-sm leading-5 text-[var(--text-secondary)]">{profile.bio}</p>
            )}
