...
const SidebarNav = memo(function SidebarNav({
  ...
  return (
      <aside
        className={`sidebar-solid fixed inset-y-3 left-3 z-50 flex w-64 flex-col rounded-[24px] shadow-2xl transition-transform duration-300 ease-out lg:translate-x-0 ${
      ...
      >
        ...
        {/* Nav — same structure for student & company (navItems already role-filtered).
            `pl-[18px]` gives the active-indicator bar (which sits outside each row,
        ...
        {/* User panel — pinned to the bottom, Discord-style: avatar + name
            ...
            right, all inside one row instead of two stacked full-width
            buttons. */}
    ...
    <div className="flex min-h-screen">
      ...
      */}
      ...
      {/* `lg:ml-[280px]` pushes this whole section right on desktop, to make
          room for the fixed-position floating sidebar (w-64 = 256px, plus
          its left-3 = 12px inset, plus 12px breathing room before content). */}
