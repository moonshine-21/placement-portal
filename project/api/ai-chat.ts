...
// Runs several "provider attempts" concurrently and resolves as soon as
// the FIRST one produces a real (truthy) result — instead of waiting for
// every one of them to finish. This is what turns Groq-then-Gemini from a
// sequential "wait for Groq's whole budget, then start Gemini's whole
// budget" (worst case: sum of both budgets) into a genuine race (worst
// case: whichever ONE of them takes longer). A provider that fails (or
// times out) just quietly drops out of the race instead of rejecting it —
// we only give up entirely if EVERY provider comes back empty/null.
// Losing providers are allowed to keep running in the background (their
// in-flight `fetch` isn't forcibly aborted), but since we've already sent
// the response by the time they finish, their result is simply ignored.
async function raceProviders(
  attempts: Array<{ label: string; run: () => Promise<string | null> }>
): Promise<{ label: string; reply: string } | null> {
  return new Promise((resolve) => {
    let remaining = attempts.length;
    let settled = false;
    attempts.forEach(({ label, run }) => {
      run()
        .then((reply) => {
          if (settled) return;
          if (reply) {
        ...
        .catch((err) => {
          console.error(`[ai-chat] [${label}] unexpected error in race:`, err);
        })
        .finally(() => {
          remaining -= 1;
...
export default async function handler(req: Request): Promise<Response> {
  ...
  // Groq and Gemini are now fired at the SAME TIME and raced against each
  // other, rather than tried one after another. Previously Gemini's whole
  // budget only started counting down after Groq's budget had *already*
  // fully elapsed — worst case ~8s + ~14s = ~22s before the user saw
  // anything. Racing them means the worst case is now just whichever ONE
  // of them is slower (~8-14s), and the common case (either one answering
  // quickly) is as fast as that provider alone, since we don't wait
  // around for the other one at all once we have a good reply.
  const attempts: Array<{ label: string; run: () => Promise<string | null> }> = [];

  if (groqKey) {
    // Groq runs open models (Llama) on its own inference chips rather
    // than GPUs, so it typically answers well under a second. Only 1
    // attempt per model (no backoff-and-retry): Groq is meant to be the
    // fast option, so if it's erroring/rate-limited, better to let
    // Gemini (racing alongside it) win than to spend time retrying.
    attempts.push({
      label: 'groq',
      run: () =>
        tryProvider(
          ...
          'groq',
          Date.now() + 8000,
          1,   // maxAttemptsPerModel — fail fast, let Gemini win the race instead
  ...
  const winner = await raceProviders(attempts);
  if (winner) {
    console.log(`[ai-chat] race won by ${winner.label}`);
    return new Response(JSON.stringify({ ai: true, reply: winner.reply, provider: winner.label }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  }
