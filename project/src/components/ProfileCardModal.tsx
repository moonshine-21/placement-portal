  ...
  return (
    ...
    >
      <div className="relative w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        {/* Close button is a sibling of the scrolling card below, not a
            child of it — positioned absolutely against THIS wrapper
            (which never scrolls), so it stays put in the same corner no
            matter how tall the card's content gets or how far it's
            scrolled. This is what fixes the button ending up scrolled out
            of view / hidden behind the header on a long profile. */}
        <button
          onClick={onClose}
          className="absolute right-3 top-3 z-20 flex h-8 w-8 items-center justify-center rounded-lg bg-black/30 text-white backdrop-blur-sm transition-colors hover:bg-black/50"
        >
          <X size={16} />
        </button>

        <div className="profile-popover max-h-[90vh] overflow-y-auto overflow-x-hidden scroll-thin animate-fade-in-scale">
        ...
        {loading ? (
          ...
          <>
            ...
            {profile.bio && (
              // Capped at a fixed max-height with its own scroll — even a
              // full 1000-character bio stays inside the card instead of
              // pushing everything below it further down the page.
              <p className="mt-5 max-h-24 overflow-y-auto scroll-thin text-sm leading-5 text-[var(--text-secondary)] whitespace-pre-wrap">{profile.bio}</p>
            )}

            {profile.skills && profile.skills.length > 0 && (
              <div className="mt-4">
                <p className="mb-2 flex items-center gap-1 text-xs font-semibold uppercase text-[var(--text-muted)]">
                  <Award size={12} /> Skills ({profile.skills.length})
                </p>
                {/* Capped height + its own scrollbar, same reasoning as the
                    bio above — a huge skill list scrolls WITHIN this box
                    rather than expanding the whole card. */}
                <div className="flex max-h-28 flex-wrap gap-1.5 overflow-y-auto scroll-thin pr-1">
                  {profile.skills.map((s) => (
                    <span key={s} className="h-fit rounded-md bg-[var(--accent)]/10 px-2 py-1 text-xs text-[var(--accent)]">
                      {s}
                    </span>
                  ))}
                </div>
