...
export function SessionGuard({ children }: { children: ReactNode }) {
  ...
  // chain of network requests into one long queue, which was actually
  // behind the multi-second delay-then-flicker on every load — not a
  // rendering/CSS bug. The overwhelming majority of visitors aren't
  // banned, so optimistically showing the app while this resolves in the
  // background, and swapping to the "Access Restricted" screen the
  // moment a ban IS confirmed, gives real (non-banned) visitors a fast
  // load without weakening the block itself — a banned visitor still
  // gets stopped, just a beat later instead of before anything else
  // could even begin loading.
  if (state === 'banned') {
    return (
