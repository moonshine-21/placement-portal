...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // `useLayoutEffect` (NOT `useEffect`) matters here: `useEffect` runs
  // AFTER the browser has already painted the frame, while
  // `useLayoutEffect` runs synchronously right after React updates the
  // DOM but BEFORE the browser paints anything on screen. The `:root`
  // CSS variables in index.css happen to default to the dark theme's
  // colors, so this was invisible for dark-theme users — but for anyone
  // on light or aurora theme, `useEffect` meant the very first painted
  // frame used the (wrong) default dark colors, then swapped to the
  // correct theme a frame later. That one-frame swap, on every single
  // page load/refresh, is a visible flash — most noticeable right where
  // the sidebar's background/border colors change. `useLayoutEffect`
  // applies the correct theme class before that first frame is ever
  // painted, so there's nothing to flash.
  useLayoutEffect(() => {
