...
export function QuizCard({ assignmentId, isMine }: { assignmentId: string; isMine: boolean }) {
  ...
  const load = async () => {
    ...
    // Look for a graded attempt regardless of what `status` says, rather
    // than only checking when status === 'completed'. The two are set in
    // the same request in api/quiz-submit.ts so they should never
    // disagree, but trusting the attempt's own existence (the thing that
    // actually determines whether "Start Quiz" or a score is the correct
    // thing to show) rather than a second flag that's supposed to mirror
    // it is one less way for a submitted quiz to still show "Start Quiz."
