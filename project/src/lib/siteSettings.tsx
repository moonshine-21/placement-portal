...
export function SiteSettingsProvider({ children }: { children: ReactNode }) {
  ...
  const load = async () => {
    ...
    setSettings((prev) => {
      const next = { site_name: data.site_name || DEFAULTS.site_name, logo_url: data.logo_url || null };
      if (prev.site_name === next.site_name && prev.logo_url === next.logo_url) return prev;
      return next;
    });
    setLoaded(true);
  };
