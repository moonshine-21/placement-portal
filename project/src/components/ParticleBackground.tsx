...
export function ParticleBackground() {
  ...
  useEffect(() => {
    ...
    // --- Flicker fix: don't let this loop run flat-out forever ---
    // This canvas sits behind the whole app and used to redraw on every
    // single display refresh (up to 60fps+), which meant anything drawn
    // ON TOP of it that samples what's beneath it (like a blurred glass
    // panel) had to be recomposited just as often. Capping this to a
    // steady ~30fps still looks perfectly smooth for slow-drifting dots,
    // but roughly halves how often the browser's compositor has any
    // reason to do that expensive work.
    const FRAME_INTERVAL = 1000 / 30;
    let lastFrameTime = 0;

    // People who've asked their OS/browser for reduced motion shouldn't
    // get a perpetually animating background at all — draw one static
    // frame and stop.
    const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    // Also fully stop the loop while the tab isn't visible (switched
    // away, minimized, etc.) — there's no point spending CPU/GPU
    // animating something nobody can see, and resuming cleanly on
    // return avoids a burst of "catch-up" work.
    ...
    const draw = (now: number = 0) => {
      // Don't schedule another frame at all once the tab is hidden — the
      // visibilitychange listener above restarts us when it becomes
      // visible again.
      if (!isVisible) return;

      // Skip this frame if we're being called sooner than our target
      // interval (requestAnimationFrame still fires at the display's
      // native rate; this just makes us do real work on roughly 1 out of
      // every 2 of those calls at 60Hz).
      ...
      // screen refresh, unless the person has asked for reduced motion —
      // in that case this single frame is the final, static one.
      if (!prefersReducedMotion) {
        animationId = requestAnimationFrame(draw);
      }
    };

    draw(); // kick off the very first frame (or the only frame, if reduced motion is on)

    // Cleanup when this component is removed from the page: stop the
    // animation loop and the resize/visibility listeners, so they don't
    // keep running forever in the background.
    return () => {
  ...
  return (
    <>
      {/* A separate plain <div> handles the solid/gradient background
          color and the custom wallpaper image (via CSS) — the canvas
          above just draws the moving particles on top of it. */}
