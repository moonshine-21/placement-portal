...
function AppContent() {
  ...
  // logged in — show a loading screen rather than flashing the wrong
  // page for a split second.
  //
  // We ALSO wait for `flagsLoaded` here, but only when there's an active
  // session (a logged-out visitor's landing/login page has no sidebar
  // and doesn't care about flags). AppShell's nav list hides flag-gated
  // items until `flagsLoaded` is true, but that guard alone isn't
  // enough — if `loading` finishes before the separate feature-flags
  // fetch does, AppShell would mount immediately with the reduced nav
  // list, then a moment later `flagsLoaded` flips true and the
  // flag-gated items (Messages, Forum, Events, etc) pop into the
  // sidebar. Waiting for both here means AppShell only ever mounts once,
  // already in its final state.
  //
  // While waiting, a logged-in visitor sees AppShellSkeleton (a
  // placeholder shaped exactly like the real sidebar/header/content —
  // see that file for why) instead of a plain unrelated spinner. That
  // matters because of an unavoidable browser fact: however fast this
  // load is, there's a real gap between "nothing painted yet" and
  // "everything painted" — React can't paint a partial frame mid-render.
  // A plain spinner screen has a totally different shape than the real
  // app, so the swap from one to the other is a visible "pop" no matter
  // how fast it happens. Shaping the placeholder like the destination
  // means there's no shape change to notice — only the skeleton's gray
  // bars becoming real content, which reads as a normal loading state
