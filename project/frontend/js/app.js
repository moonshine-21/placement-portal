...
function switchView(view) {
  ...
  // Search bar is only relevant on the Companies tab (students) / Applicants
  // tab (companies) — hide it everywhere else.
  const searchWrap = document.getElementById("searchWrap");
  if (searchWrap) searchWrap.hidden = !(view === "companies" || view === "applicants");
}
...
function initSearch() {
  ...
  async function performSearch(query) {
    ...
    const { data: users } = await supabase
      .from("profiles")
      .select("id, full_name, email, avatar_url, role, branch, skills, cgpa, profile_completion, bio")
...
function openMiniProfile(entity, kind) {
  ...
  // NOTE: student/user email is intentionally NOT shown here — emails are
  // private and only visible internally via auth, never on public profiles.
...
async function updateMiniProfileFriendButton(otherId) {
  ...
  try {
    ...
    if (!data) {
      ...
      wrap.innerHTML = `<button class="btn-ghost btn-sm" disabled style="opacity:0.7">✓ Friends</button><button class="btn-primary btn-sm" id="mpCallBtn" style="margin-left:0.5rem">📞 Call</button>`;
      const callBtn = document.getElementById("mpCallBtn");
      if (callBtn) callBtn.addEventListener("click", () => {
        closeMiniProfile();
        startCall(otherId, "friend");
      });
...
async function loadConversations() {
  ...
  if (otherIds.length) {
    const { data: profs } = await supabase.from("profiles").select("id, full_name, avatar_url, role, branch, skills, cgpa, profile_completion, bio").in("id", otherIds);
...
async function loadAcceptedFriends() {
  ...
  try {
    const { data: incoming, error: e1 } = await supabase
      .from("friends")
      .select("id, requester_id, profiles!friends_requester_id_fkey(full_name, avatar_url, branch, bio)")
      .eq("recipient_id", currentUser.id)
      .eq("status", "accepted");
    if (e1) throw e1;
    const { data: outgoing, error: e2 } = await supabase
      .from("friends")
      .select("id, recipient_id, profiles!friends_recipient_id_fkey(full_name, avatar_url, branch, bio)")
      .eq("requester_id", currentUser.id)
      .eq("status", "accepted");
    if (e2) throw e2;
    const friends = [
      ...
      .map((f) => {
        ...
        return `<div class="friend-row">
          ...
          <div class="friend-row-body">
            ...
            <div class="friend-row-sub">${escapeHtml(f.branch || "")}</div>
          </div>
          <div class="friend-row-actions">
...
async function prepareCall(callId, isCaller) {
  ...
  try {
    // Camera is optional: try audio+video first, but if the user denies the
    // camera (or has none) fall back to audio-only instead of failing the
    // whole call.
    try {
      localMediaStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
    } catch (mediaErr) {
      console.warn("Camera unavailable, falling back to audio-only", mediaErr);
    ...
    rtcPeer.ontrack = (e) => {
      // Assign the remote stream as soon as it's negotiated, but do NOT mark
      // the call "In call" here: ontrack can fire before ICE actually
      // connects, which would falsely show "In call" with no audio. The
      // connected state is handled in onconnectionstatechange below.
