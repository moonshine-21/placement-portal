...
function AppContent() {
  ...
  // Set when a student clicks "View & Apply" on a Matches card — tells
  // CompaniesBrowseView which company to jump straight to instead of
  // making them search for it again.
  ...
  const renderView = () => {
    switch (effectiveView) {
      ...
      case 'matches': return <MatchesView onNavigate={(v) => setCurrentView(v as ViewKey)} onApplyToCompany={setPendingCompanyId} />;
      case 'applications': return <ApplicationsView />;
      case 'companies': return <CompaniesBrowseView onNavigate={(v) => setCurrentView(v as ViewKey)} preselectCompanyId={pendingCompanyId} onPreselectConsumed={() => setPendingCompanyId(null)} />;
