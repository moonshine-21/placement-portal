...
## Full glass pass (2026-08-25)
- Restored real `backdrop-filter` glass on the sidebar, header, main workspace, cards and glass utilities.
- Kept blur static and isolated with `contain: paint`/`isolation: isolate`; never use `will-change: backdrop-filter`.
- Particle canvas remains the only continuously animated layer.
- No blur/filter transitions are used.
- Header remains flush and rectangular against the sidebar.
