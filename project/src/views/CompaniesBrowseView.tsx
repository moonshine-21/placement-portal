...
import { Building2, ArrowLeft, Globe, Mail, Phone, MapPin, Briefcase, Send, X, Bookmark, BookmarkCheck } from 'lucide-react';
  ...
  return (
    <div className="card">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-sm text-[var(--text-muted)]">{companies.length} companies</span>
      </div>
      {companies.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-12 text-center">
          <Building2 size={32} className="text-[var(--text-muted)]" />
          <p className="text-sm text-[var(--text-muted)]">No company profiles yet.</p>
        </div>
      ) : (
