...
import type { Profile, Company, Match, CompanyProfile, Job } from './supabase';
...
// ----------------------------------------------------------------------
// LIVE MATCHING against the REAL company system (company_profiles + jobs)
// ----------------------------------------------------------------------
//
// The computeMatch/generateAndSaveMatches pair above only ever worked
// against the older, separate `companies` table — which was deliberately
// emptied out (see the "remove demo companies" migration) and is no
// longer how companies (including the AI/"bot" ones) actually get added
// to the platform. Real companies live in `company_profiles`, with their
// open roles in `jobs` — this second pair of functions matches a student
// against THOSE instead, computed fresh on every call (not persisted to
// the `matches` table, since that table's `company_id` has a database
// foreign key locked to the old `companies` table specifically and can't
// reference `company_profiles` rows without a schema migration).
//
// One open job = one row here (rather than one row per company), since
// package/role/skills are really properties of a specific job posting,
// not the company as a whole — a company with 3 open roles shows up as
// up to 3 separate match rows, one per role.

export type LiveMatch = {
  id: string; // the job's id — stable + unique, usable as a React key
  match_score: number;
...
// Scores one job against a student's profile. Unlike computeMatch above,
// there's no CGPA/branch component — company_profiles/jobs simply don't
// store a minimum CGPA or an eligible-branches list (those only ever
// existed on the old `companies` table), so this scores purely on skill
// overlap (up to 80 points) plus how complete the student's profile is
// (up to 20 points).
function computeJobMatch(profile: Profile, job: Job): { score: number; matched: string[]; missing: string[]; reasoning: string } {
  const studentSkills = profile.skills || [];
  ...
  let score = requiredSkills.length > 0 ? Math.round((matched.length / requiredSkills.length) * 80) : 40;
  score += profile.profile_completion >= 80 ? 20 : Math.round((profile.profile_completion / 80) * 20);
  score = Math.min(100, score);

  const reasoning = missing.length === 0
    ? `You meet every listed skill for this role.`
    : `Good match — picking up ${missing.slice(0, 3).join(', ')} would make you a stronger fit.`;

  return { score, matched, missing, reasoning };
}

// Loads every currently-open job across every company, scores each one
// against the given student profile, and returns them best-match-first.
export async function loadLiveMatches(profile: Profile): Promise<LiveMatch[]> {
  ...
  if (jobsErr) {
    console.error('loadLiveMatches (jobs) failed:', jobsErr);
    return [];
  }
  if (!jobs || jobs.length === 0) return [];

  // `jobs.company_id` and `company_profiles.id` aren't linked by a direct
  // foreign key (both separately reference auth.users), so Supabase can't
  // auto-embed them in one query — fetch company_profiles separately and
  // join them together here in JS instead.
  const companyIds = [...new Set((jobs as Job[]).map((j) => j.company_id))];
  ...
  if (compErr) {
    console.error('loadLiveMatches (company_profiles) failed:', compErr);
    return [];
  }

  const companyMap = new Map((companies as CompanyProfile[] || []).map((c) => [c.id, c]));

  const results: LiveMatch[] = [];
  for (const job of jobs as Job[]) {
    const company = companyMap.get(job.company_id);
    if (!company || !company.org_name) continue; // skip jobs whose company profile is missing or still a blank "shell" account
    const { score, matched, missing, reasoning } = computeJobMatch(profile, job);
    results.push({ id: job.id, match_score: score, matched_skills: matched, missing_skills: missing, reasoning, company, job });
  }
