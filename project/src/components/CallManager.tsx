...
export function CallManager({ startCallRequest, onCallConsumed }: Props) {
  ...
  const prepareCall = async (id: string, isCaller: boolean, otherProfile?: { name: string; avatar: string }) => {
    ...
    // Accumulate remote tracks so audio is not dropped when video arrives separately
    ...
    peer.ontrack = (e) => {
      ...
      // IMPORTANT: this assignment only works if the <video> element for
      // `remoteVideoRef` already exists in the DOM at this exact moment.
      // It only renders while `state === 'active'` (see the JSX at the
      // bottom of this file) — but for the CALLER specifically, this
      // `ontrack` event can fire WHILE they're still in the 'outgoing'
      // (ringing) state, because flipping to 'active' depends on a
      // completely separate signal (the `calls` DB row's status turning
      // 'accepted', received over its own realtime channel) that isn't
      // guaranteed to arrive before the actual WebRTC media does. If that
      // race is lost, `remoteVideoRef.current` is null here and this
      // assignment is silently skipped — with nothing to retry it later,
      // which is exactly why callers (but not callees, who always flip to
      // 'active' before their connection starts negotiating — see
      // acceptCall()) could end up with a working connection but a blank
      // video. The `useEffect` below, which re-attaches `remoteStreamRef`
      // whenever `state` becomes 'active', is what actually guarantees
      // this works regardless of which order these two things happen in.
      ...
      if (el) {
        ...
        el.volume = 1;
        el.play().catch(() => {
          // Autoplay may block until user gesture; try again shortly
  ...
  // The <video> elements for local/remote preview only exist in the DOM
  // while `state === 'active'` (see the JSX below). But the actual media
  // streams (local, from getUserMedia; remote, from the peer's `ontrack`
  // — see prepareCall() above) can become ready BEFORE that, especially
  // for the caller, whose `state` only becomes 'active' once a separate,
  // independent realtime signal (the `calls` row turning 'accepted')
  // arrives — which isn't guaranteed to win the race against the actual
  // WebRTC media arriving. Re-running this attachment every time `state`
  // changes to 'active' guarantees both video elements always end up
  // showing whatever stream is currently available, regardless of
  // whether that stream became ready before or after the elements
  // mounted — closing the exact gap that let the caller end up with a
  // working connection (audio, "In call" status) but a blank remote
  // video, while the callee (who always flips to 'active' before their
  // connection starts negotiating — see acceptCall()) never had this
  // problem.
  useEffect(() => {
