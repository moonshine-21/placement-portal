...
function SendQuizModal({ quiz, onClose }: { quiz: QuizListItem; onClose: () => void }) {
  ...
  // "Send to all": sends this quiz to every applicant who doesn't already
  // have it. Applicants who already have an assignment (sent / in progress /
  // completed) are skipped, so nobody ever gets a duplicate. Runs in small
  // parallel batches so a big applicant list doesn't fire hundreds of
  // requests at once.
  const [sendingAll, setSendingAll] = useState(false);
  const [confirmAll, setConfirmAll] = useState(false);
  const [progress, setProgress] = useState({ done: 0, total: 0 });
  const pendingStudents = students.filter((s) => !statusByStudent[s.id]);

  const sendAll = async () => {
    if (!profile || pendingStudents.length === 0) return;
    ...
    for (let i = 0; i < targets.length; i += BATCH) {
      ...
      await Promise.all(batch.map(async (student) => {
        const { error, alreadySent, existingStatus } = await sendQuizToStudent(
          profile.id, profile.full_name || 'A company', student.id, quiz.id, quiz.title
        );
        if (error) { failed++; }
        else {
          ok++;
          setStatusByStudent((st) => ({ ...st, [student.id]: alreadySent ? (existingStatus || 'sent') : 'sent' }));
        }
        setProgress((p) => ({ ...p, done: p.done + 1 }));
      }));
    }
    setSendingAll(false);
    if (failed === 0) showToast(`Quiz sent to ${ok} applicant${ok === 1 ? '' : 's'}`, 'success');
    else showToast(`Sent to ${ok}, failed for ${failed}. You can press "Send to all" again to retry the rest.`, 'error');
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="modal-panel w-full max-w-sm p-5" onClick={(e) => e.stopPropagation()}>
        ...
        {!loading && students.length > 0 && (
          <div className="mb-3">
            {confirmAll ? (
              <div className="flex items-center justify-between gap-2 rounded-lg border border-[var(--border-strong)] px-3 py-2">
                <span className="text-xs text-[var(--text-secondary)]">Send to {pendingStudents.length} applicant{pendingStudents.length === 1 ? '' : 's'}?</span>
                <div className="flex gap-2">
                  <button onClick={() => setConfirmAll(false)} className="btn-ghost btn-sm">Cancel</button>
                  <button onClick={sendAll} className="btn-primary btn-sm">Yes, send</button>
                </div>
              </div>
            ) : (
              <button
                onClick={() => setConfirmAll(true)}
                disabled={sendingAll || pendingStudents.length === 0}
              ...
              >
                ...
                {sendingAll
                  ...
                  : pendingStudents.length === 0
                    ? 'Already sent to all applicants'
                    : `Send to all (${pendingStudents.length})`}
              </button>
            )}
          </div>
        )}
            ...
            filtered.map((s) => {
              ...
              return (
                <div key={s.id} className="flex items-center justify-between gap-2 rounded-lg px-2 py-2 hover:bg-[var(--surface-hover)]">
                  ...
                  <button
                    onClick={() => send(s)}
                    disabled={sendingTo === s.id || sendingAll || !!status}
