    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <form onSubmit={createPost} className="card space-y-4 animate-slide-up">
          ...
          <div><label className="mb-2 block text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Category</label><Select value={category} onChange={setCategory} options={CATEGORIES.map(c => ({ value: c.key, label: c.label }))} /></div>
