...
export function ParticleBackground() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const count = Math.min(Math.floor((width * height) / 30000), 55);
      ...
      particles = Array.from({ length: count }, () => ({
        ...
        vx: (Math.random() - 0.5) * 0.18, // random value between -0.15 and +0.15
    ...
    const draw = () => {
      ...
      const color = colorMap[t] || '111, 107, 250';
      ...
      particles.forEach((p, i) => {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 115) {
            ...
            ctx.strokeStyle = `rgba(${color}, ${(1 - dist / 115) * 0.055})`;
            ctx.lineWidth = 0.6;
            ctx.stroke();
          }
        }
  ...
  // Particles re-enabled: the earlier shimmer came from backdrop-filter blur
  // (on the sidebar/cards) re-sampling these moving pixels every frame, not
  // from the canvas animation itself. Now that surfaces use a static
  // translucent color instead of live blur, the canvas runs independently
