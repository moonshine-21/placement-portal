...
function AppContent() {
  ...
  console.log('[flicker-debug] AppContent rendering at', performance.now().toFixed(0) + 'ms', '— loading:', loading, 'has session:', !!session, 'has profile:', !!profile, 'role:', profile?.role);
  ...
  // Same pattern as pendingConvUser above, but for "View & Apply" on the
  // Matches page — carries which company AND which job to open straight
  ...
  const renderView = () => {
    switch (effectiveView) {
      ...
      case 'matches': return <MatchesView onViewCompany={(company, job) => { setPendingCompanyView({ company, job }); setCurrentView('companies'); }} />;
  ...
  return (
    <>
      ...
      <AppShell currentView={effectiveView} onNavigate={(v) => setCurrentView(v)} onSignOut={handleSignOut}>
        {renderView()}
        {/* AIAssistantView is deliberately NOT part of the renderView()
            switch above. Every other view gets fully UNMOUNTED the
            instant you navigate away (the switch just stops rendering
            it) — fine for a static page, but disastrous for a chat
            that's mid-request: unmounting kills the component's state,
            so if Gemini/the backend was still "typing" a reply when you
            clicked over to the Dashboard, that reply had nowhere to
            land and just vanished — the assistant looked like it had
            silently stopped working.
            Fix: once a student has opened the AI Assistant, keep this
            component mounted permanently for the rest of the session —
            same "always alive, just hidden" trick already used for
            CallManager below — and only toggle its CSS visibility based
            on whether 'ai-assistant' is the active tab. The chat, its
            in-flight request, and the typing indicator now keep running
            in the background no matter which tab is on screen, and
            switching back shows it exactly as it currently stands. */}
