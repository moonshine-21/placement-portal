...
// The shape of one saved chat bubble — kept as its own named type since
// it's now used in three places (component state, and the two localStorage
// helpers below), not just inline in `useState` like before.
type StoredChatMessage = { role: 'bot' | 'user'; text: string; source?: 'gemini' | 'backend' | 'offline'; debug?: string };

// The localStorage key each user's saved conversation is kept under —
// suffixed with their own user id, so different accounts on the same
// browser never see each other's chat history.
const CHAT_STORAGE_PREFIX = 'spc-ai-chat-';
// How long a conversation is kept around with no activity before it's
// treated as stale and cleared — 30 minutes, per the "unless the user
// stays inactive for 30 mins" requirement.
const CHAT_INACTIVITY_LIMIT_MS = 30 * 60 * 1000;

// Reads this user's saved conversation back out of localStorage, if there
// is one AND it's still "fresh" (last touched within the 30-minute
// window above). Returns `null` for "there's nothing usable to restore" —
// covering a brand new user, an expired conversation, or a browser with
// localStorage unavailable/blocked (private browsing, storage full,
// etc.) — in every one of those cases, the caller just starts a normal
// fresh conversation instead, so this never breaks the page.
function loadStoredChat(userId: string | null | undefined): { messages: StoredChatMessage[]; showSuggestions: boolean } | null {
  if (!userId) return null;
  try {
    const raw = localStorage.getItem(CHAT_STORAGE_PREFIX + userId);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as { messages?: StoredChatMessage[]; lastActivity?: number; showSuggestions?: boolean };
    if (!parsed.lastActivity || Date.now() - parsed.lastActivity > CHAT_INACTIVITY_LIMIT_MS) {
      // Stale — the person has been away for 30+ minutes. Clear it out
      // rather than leaving a dead entry sitting in localStorage forever.
      localStorage.removeItem(CHAT_STORAGE_PREFIX + userId);
      return null;
    }
    if (!Array.isArray(parsed.messages) || parsed.messages.length === 0) return null;
    return { messages: parsed.messages, showSuggestions: parsed.showSuggestions ?? false };
  } catch {
    return null; // corrupted JSON, storage blocked, etc. — fail quietly, start fresh
  }
}

// Saves this user's current conversation to localStorage, stamped with
// "right now" as the last-activity time — called every time the
// conversation changes (see the effect below), which is what keeps
// refreshing the 30-minute inactivity window as long as the person is
// actually using the chat.
function saveStoredChat(userId: string, messages: StoredChatMessage[], showSuggestions: boolean) {
  try {
    localStorage.setItem(CHAT_STORAGE_PREFIX + userId, JSON.stringify({ messages, showSuggestions, lastActivity: Date.now() }));
  } catch {
    // Storage full/blocked — the chat just won't survive a refresh this
    // time, which is a much better failure mode than crashing the page.
  }
}
...
export function AIAssistantView() {
  ...
  //
  // ---- Persistence: why the chat used to disappear ----
  // This whole `messages` list used to live ONLY in this component's React
  // state, with nowhere else it was ever saved. Switching to a different
  // page (which unmounts this component) or refreshing the browser (which
  // wipes all JS state) both reset it straight back to empty — that's the
  // exact "my chat gets deleted" bug. The fix below saves the conversation
  // to the browser's own `localStorage` (a small storage box tied to this
  // site, kept per browser, separate from anything on our server) every
  // time it changes, tagged with a `lastActivity` timestamp and the
  // signed-in user's own id (so switching accounts never shows someone
  // else's conversation). Coming back to this page — even after a full
  // refresh — restores it, UNLESS more than 30 minutes have passed since
  // the last activity, in which case it's treated as stale and a fresh
  // conversation starts instead (see `loadStoredChat` below).
  const [messages, setMessages] = useState<StoredChatMessage[]>(() => loadStoredChat(user?.id)?.messages ?? []);
  const [input, setInput] = useState('');
  const [typing, setTyping] = useState(false); // shows the "..." typing-indicator bubble while waiting for a reply
  const [showSuggestions, setShowSuggestions] = useState(() => loadStoredChat(user?.id)?.showSuggestions ?? true);
  ...
  // but only if there was no valid saved conversation to restore (the
  // `messages` initializer above already populated `messages` from
  // localStorage if one existed and was still fresh, so this correctly
  ...
  // Whenever the conversation changes, save it back to localStorage
  // (refreshing the `lastActivity` timestamp each time) — this is what
  // actually keeps it alive across a page switch or refresh. Skipped
  // while `messages` is still empty (nothing worth saving yet, and avoids
  // overwriting a real saved conversation with an empty one during the
  // very first render before the welcome-message effect above runs).
  useEffect(() => {
    if (user?.id && messages.length > 0) {
      saveStoredChat(user.id, messages, showSuggestions);
    }
  }, [messages, showSuggestions, user?.id]);
