...
export function CallManager({ startCallRequest, onCallConsumed }: Props) {
  ...
  const getIceServers = async (): Promise<RTCIceServer[]> => {
    try {
      ...
      if (res.ok) {
        ...
        if (data?.note) console.warn('[calls] No TURN servers:', data.note);
      } else {
        console.warn('[calls] /api/turn-credentials returned', res.status);
      }
    } catch (err) {
      console.warn('[calls] Could not fetch TURN credentials:', err);
    }
  ...
  const attachRemoteVideo = () => {
    ...
    // IMPORTANT: don't bind (or mark "ready") on a stream that doesn't have
    // any tracks on it yet. `remoteStreamRef.current` is created as an
    // empty `new MediaStream()` the moment `prepareCall` runs — for the
    // CALLER that happens immediately when the call starts (long before
    // the callee has even answered), so this function can get called
    // (from the `state === 'active'` mount effect) while that stream is
    // still empty. A `MediaStream` object is truthy even with zero tracks,
    // so without this check we'd bind the empty stream, call `.play()`,
    // and flip `remoteReady` to true right away — hiding the "Connecting…"
    // placeholder and showing a blank black video. Worse, several browsers
    // never repaint a `<video>` after tracks are added to a stream it's
    // *already* bound to (no-op on `srcObject !==` check below) — so once
    // the real remote track finally arrived via `ontrack`, nothing visibly
    // updated. This is why callers could see the call go "active" with no
    // audio/video while callees (who only ever bind once real tracks
    // exist) worked fine. Waiting for real tracks here — and letting
    // `ontrack` force a fresh bind (see below) — fixes both sides.
    if (!el || !stream || stream.getTracks().length === 0) return;
    if (el.srcObject !== stream) el.srcObject = stream;
    el.muted = false;
    el.volume = 1;
    el.playsInline = true;
  ...
  const prepareCall = async (id: string, isCaller: boolean) => {
    ...
    try {
      ...
      try {
        ...
        // No camera AND no mic — the call can technically still connect
        // (you'd still receive the other person's audio/video), but you
        // won't be sending anything, which looks exactly like "the call
        // connects but nothing works" from the other side. Surface this
        // clearly instead of failing silently.
        console.warn('getUserMedia failed completely:', err);
        showToast('Could not access camera/microphone — check your browser permissions', 'error');
      }
    }
    ...
    peer.ontrack = (e) => {
      ...
      remoteStreamRef.current = remoteStream;
      // Force a fresh `srcObject` bind now that the stream actually has
      // media on it. If an earlier, empty bind already happened (see the
      // big comment in attachRemoteVideo above), `el.srcObject === stream`
      // already and attachRemoteVideo's own re-bind would be skipped as a
      // no-op — clearing it first guarantees the browser re-evaluates the
      // stream and actually renders the newly-added track(s).
      const el = remoteVideoRef.current;
    ...
    peer.onconnectionstatechange = () => {
      ...
      if (cs === 'connected') {
        ...
        // Safety net for the "caller is stuck on the ringing screen while
        // the callee is already fully connected" bug: normally the CALLER
        // only swaps its UI from the outgoing ring screen to the full
        // active-call screen once it hears — over Supabase Realtime —
        // that the `calls` row flipped to 'accepted' (see
        // subscribeCallStatus below). Realtime has to be separately
        // switched on per-table (it's not implied by RLS policies); if
        // that was never enabled for the `calls` table, that event never
        // reaches the caller, so their screen never advances even though
        // the actual WebRTC connection — which doesn't depend on that
        // Realtime row at all — connects just fine peer-to-peer. That
        // produces exactly the reported symptom: the callee sees/hears
        // everything, the caller sees nothing and is stuck on "Calling…".
        // Since we already know for certain the call is connected the
        // moment we get here, force the UI into the active state directly
        // instead of relying solely on that Realtime signal.
        setState('active');
