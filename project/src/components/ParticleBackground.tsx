...
export function ParticleBackground() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      // Visible constellation without being heavy enough to stress the GPU.
    ...
    const draw = (now: number) => {
      ...
      if (!reduceMotion && now - lastFrame < 33) return; // ~30fps
      lastFrame = now;
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distSq < 140 * 140) {
            const alpha = (1 - Math.sqrt(distSq) / 140) * 0.12;
            ctx.beginPath();
            ctx.moveTo(p.x, p.y);
            ctx.lineTo(q.x, q.y);
    ...
    const onVisibility = () => {
      ...
      if (!running) {
        ...
        // The draw loop bails out (without scheduling its next frame) the
        // moment the tab goes hidden, so coming back to the tab needs to
        // explicitly kick the RAF chain off again — otherwise the canvas
        // would stay frozen forever after the first tab-away/tab-back.
        lastFrame = 0;
        raf = requestAnimationFrame(draw);
      }
    };
