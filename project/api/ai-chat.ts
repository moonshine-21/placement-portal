...
//
// This exists so the Gemini API key never ships to the browser. The reason
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!apiKey) {
    console.error('[ai-chat] No Gemini API key found — checked GEMINI_API_KEY and VITE_GEMINI_API_KEY');
    return new Response(JSON.stringify({ ai: false, error: 'No Gemini API key configured on the server (checked GEMINI_API_KEY and VITE_GEMINI_API_KEY) — add one in Vercel → Project Settings → Environment Variables, then redeploy' }), { status: 200 });
  }
