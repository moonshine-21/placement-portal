...
function initAuthPage() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (error) {
      ...
      if (/rate limit|too many|over the limit|for security purposes|429/i.test(msg)) {
        showError("Too many attempts in a short time. Wait a minute and try again.");
      } else if (/already registered|already exists|user already registered/i.test(msg)) {
        showError("An account with this email already exists. Please sign in instead.");
      } else {
        showError(msg);
      }
  ...
  if (forgotBtn) {
    forgotBtn.addEventListener("click", async () => {
      ...
      if (error) {
        // Rate-limit / transport errors still leak that the email is valid,
        // so Supabase returns a generic message. Surface it but also tell
        // the user a link may have been sent to avoid revealing account state.
        if (/rate limit|too many|over the limit/i.test(error.message)) {
          showError("Too many reset emails sent recently. Wait a few minutes and try again.");
        } else {
...
const VIEW_META = {
  ...
  friends: { t: "Friends", s: "Connect with students and call your friends" },
};
...
/* =========================================================
   RENDER — DASHBOARD
   ========================================================= */
async function updateDashboardTrends(current) {
  ...
  try {
    ...
    await supabase.from("dashboard_snapshots").upsert({
      ...
      companies_matched: current.companies_matched,
      high_matches: current.high_matches,
      shortlisted: current.shortlisted,
    }, { onConflict: "student_id,snapshot_date" });

    setTrend("matchTrend", prev ? current.match_score - prev.match_score : null, "%");
    setTrend("companiesTrend", prev ? current.companies_matched - prev.companies_matched : null, "");
    setTrend("highTrend", prev ? current.high_matches - prev.high_matches : null, "%");
    setTrend("shortlistedTrend", prev ? current.shortlisted - prev.shortlisted : null, "");
  } catch (err) {
    console.error("updateDashboardTrends failed:", err);
    ["matchTrend", "companiesTrend", "highTrend", "shortlistedTrend"].forEach((id) => { if (el(id)) el(id).textContent = "–"; });
  }
}

function setTrend(id, delta, suffix) {
  const el = document.getElementById(id);
  if (!el) return;
  if (delta === null || delta === undefined) { el.textContent = "New"; el.className = "stat-trend"; return; }
  if (delta === 0) { el.textContent = "– 0" + suffix; el.className = "stat-trend"; return; }
  const up = delta > 0;
  el.textContent = (up ? "▲ " : "▼ ") + Math.abs(delta) + suffix;
...
async function loadShortlistedCount() {
  ...
  try {
    const [coRes, legacyRes] = await Promise.all([
      supabase.from("company_applications").select("id", { count: "exact", head: true }).eq("student_id", currentUser.id).eq("status", "shortlisted"),
      supabase.from("applications").select("id", { count: "exact", head: true }).eq("student_id", currentUser.id).eq("status", "shortlisted"),
    ]);
    return (coRes.count || 0) + (legacyRes.count || 0);
  } catch (err) {
    console.error("loadShortlistedCount failed:", err);
    return 0;
  }
}

function renderDashboard(matches, shortlistedCount) {
  allMatches = matches || [];
  shortlistedCount = shortlistedCount || 0;
  if (!allMatches.length) {
    renderEmptyDashboard(shortlistedCount);
    return;
  }
  ...
  animateCount("shortlisted", shortlistedCount, 900);
  ...
  document.getElementById("shortBar").style.setProperty("--w", (allMatches.length ? (shortlistedCount / allMatches.length) * 100 : 0) + "%");
  renderCompanyTable(allMatches);
  renderWhyList(allMatches);
  renderAIRec(allMatches);
  renderProfileCompletion();
  buildCharts(allMatches);
  updateDashboardTrends({ match_score: avg, companies_matched: allMatches.length, high_matches: high, shortlisted: shortlistedCount });
}

function renderEmptyDashboard(shortlistedCount) {
...
function renderAIRec(matches) {
  ...
  if (apply) apply.onclick = () => applyToCompany(c.id);
}
...
// inflates file size by ~33% — so keep the encoded payload under ~3.2MB raw.
const MAX_UPLOAD_BYTES = 3.2 * 1024 * 1024;

async function handleUpload(file) {
  const result = document.getElementById("uploadResult");

  // PDFs (and anything non-image) can't be auto-shrunk client-side — fail fast
  // with a clear message instead of burning a request on a guaranteed 413.
  if (!file.type.startsWith("image/") && file.size > MAX_UPLOAD_BYTES) {
    result.innerHTML = `<div class="empty-state"><div class="icon">⚠</div><p>This file is too large (${(file.size / 1024 / 1024).toFixed(1)}MB).</p><p class="muted" style="margin-top:0.5rem;font-size:0.82rem">Please upload a PDF under ~3MB — try compressing it (e.g. smallpdf.com/compress-pdf) or exporting fewer pages.</p></div>`;
    const input = document.getElementById("fileInput");
  ...
  const { analysis, error } = await analyzeWithGemini(file);
  if (!analysis) {
    console.error("analyze-resume failed:", error);
    result.innerHTML = `<div class="empty-state"><div class="icon">⚠</div><p>Couldn't analyze this document.</p><p class="muted" style="margin-top:0.5rem;font-size:0.82rem;word-break:break-word">${error || "Unknown error"}</p></div>`;
    const input = document.getElementById("fileInput");
    if (input) input.value = "";
    return;
  }
...
async function analyzeWithGemini(file) {
  ...
  try {
    if (file.type.startsWith("image/")) {
      // Phone-camera photos of marksheets/resumes are often 4-8MB — resize +
      // recompress client-side so the upload reliably fits the payload limit.
    ...
    return { analysis: null, error: `Couldn't read/compress file: ${err.message}` };
  }
  ...
  try {
    res = await fetch("/api/analyze-resume", {
      ...
      body: JSON.stringify({ file_base64: base64, mime_type: mimeType, file_name: file.name }),
    });
  } catch (err) {
    return { analysis: null, error: `Network error: ${err.message}` };
  }
  let data = null;
  try {
    data = await res.json();
    ...
    if (res.status === 413) {
      return { analysis: null, error: "File is still too large after compression. Please try a smaller image or a PDF under ~3MB." };
    }
    return { analysis: null, error: `HTTP ${res.status} — non-JSON response (likely a payload-size or platform error, not Gemini itself)` };
  }
  if (!res.ok) {
    return { analysis: null, error: `HTTP ${res.status}: ${data?.error || JSON.stringify(data)}` };
  }
  if (!data.ai) {
    return { analysis: null, error: `${data.reason || "unknown"}${data.detail ? " — " + data.detail : ""}` };
  }
  return { analysis: data.analysis || null, error: null };
}

// Resizes to a max dimension and recompresses as JPEG, stepping quality down
// until the base64 payload fits under MAX_UPLOAD_BYTES.
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const tryEncode = () => {
        ...
        if (approxBytes > MAX_UPLOAD_BYTES && quality > 0.3) {
          quality -= 0.15;
    ...
    img.onerror = () => { URL.revokeObjectURL(objectUrl); reject(new Error("Couldn't load image")); };
    img.src = objectUrl;
  });
}
...
async function regenerateMatches() {
  ...
  const shortlistedCount = await loadShortlistedCount();
  renderDashboard(allMatches, shortlistedCount);
...
async function getAIReply(question) {
  ...
  // the key must NEVER live in this browser-side file.
  try {
    ...
    if (res.ok) {
      ...
      // The function responded but AI is off — tell the user WHY, don't hide it.
      if (data.reason === "no_api_key") {
        return `<strong>AI is not connected yet.</strong><br><br>The Gemini API key isn't set in Vercel. Add <strong>GEMINI_API_KEY</strong> under Project → Settings → Environment Variables, then redeploy. Meanwhile, here's a quick answer:<br><br>${ruleBasedReply(question)}`;
      }
      if (data.reason === "upstream_error") {
        const detail = data.detail ? `<br><br><small>Detail: ${data.detail}</small>` : "";
        return `<strong>The AI service returned an error.</strong><br><br>Your Gemini key may be invalid, expired, or the free quota is exhausted. Check the <strong>GEMINI_API_KEY</strong> value in Vercel and that the Gemini API is enabled for your key.${detail}<br><br>Meanwhile:<br><br>${ruleBasedReply(question)}`;
      }
      if (data.reason === "empty_response") {
        return `<strong>The AI had no answer for that.</strong><br><br>Try rephrasing. Meanwhile:<br><br>${ruleBasedReply(question)}`;
      }
      // Unknown reason — surface it.
      return `<strong>AI couldn't respond (reason: ${data.reason || "unknown"}).</strong><br><br>Meanwhile:<br><br>${ruleBasedReply(question)}`;
    }
    return `<strong>The AI service is unreachable (HTTP ${res.status}).</strong><br><br>Make sure the app is deployed on Vercel so the <strong>/api/ai-chat</strong> function is available. Meanwhile:<br><br>${ruleBasedReply(question)}`;
  } catch (err) {
    // Network error — the /api function isn't deployed at all (e.g. local Vite only).
    return `<strong>AI service isn't available.</strong><br><br>The <strong>/api/ai-chat</strong> function only runs on Vercel — it isn't available in local dev. Deploy to Vercel and set <strong>GEMINI_API_KEY</strong> to enable it. Meanwhile:<br><br>${ruleBasedReply(question)}`;
  }
}
...
function initSearch() {
  ...
  async function performSearch(query) {
    const results = [];
    // Always search other users by name (excludes self + company-role accounts,
    // which are shown via company_profiles instead).
    const { data: users } = await supabase
      .from("profiles")
      .select("id, full_name, email, avatar_url, role, branch, skills, cgpa, profile_completion")
    ...
    (users || []).forEach((u) => {
      results.push({
        ...
        bio: u.branch ? `${u.branch}${u.cgpa ? " · CGPA " + u.cgpa : ""}` : (u.email || ""),
        type: "user",
        id: u.id,
    ...
    if (currentProfile?.role === "company") {
      // Company also searches its applicants
      const { data: apps } = await supabase
        ...
        .ilike("full_name", `%${query}%`);
      (apps || []).forEach((a) => {
        if (!results.find((r) => r.id === a.student_id)) {
          results.push({ name: a.full_name, bio: a.email, contact: a.phone, type: "student", id: a.student_id, status: a.status });
        }
      });
    } else {
      // Student also searches companies by name
      const { data: comps } = await supabase
        .from("company_profiles")
        .select("*")
        .ilike("org_name", `%${query}%`);
      (comps || []).forEach((c) => {
        results.push({ name: c.org_name, bio: c.industry || (c.about_us || "").slice(0, 80) || "", contact: c.contact_email, type: "company", id: c.id, avatar_url: c.avatar_url, obj: c });
      });
    }
  ...
  function renderSearchResults(results, query) {
    ...
    searchResults.innerHTML = results.map((r) => {
      ...
      const primaryLabel = isCompany ? "View & Contact" : "View Profile";
      const primaryAttr = isCompany ? `data-view-company="${r.id}"` : `data-view-user="${r.id}"`;
      return `
        <div class="search-result-row">
          ...
          <div class="search-result-contact">
            <button class="btn-primary btn-sm" ${primaryAttr}>${primaryLabel}</button>
            ${isUser ? `<button class="btn-ghost btn-sm" data-msg-user="${r.id}">Message</button>` : ""}
          </div>
        </div>`;
    }).join("");
    ...
    searchResults.querySelectorAll("[data-view-user]").forEach((b) => b.addEventListener("click", () => {
      const u = results.find((x) => x.id === b.dataset.viewUser);
      if (u?.obj) openMiniProfile(u.obj, "user");
      searchResults.remove(); searchResults = null; input.value = "";
    }));
...
async function initDashboard() {
  ...
  subscribeCalls();
  // Friend requests must also be heard app-wide. Without this, a new
  // incoming request updates the notifications panel (which subscribes at
  // init) but never reaches the Friends sidebar unless the recipient has
  // that tab open — because subscribeFriends() only ran from loadFriendsView().
  subscribeFriends();
...
async function loadCompanyOverview() {
  ...
  // Hiring progress — sum across the company profile + all jobs.
  ...
  const { data: jobs } = await supabase.from("jobs").select("employees_needed, employees_have").eq("company_id", currentUser.id);
  let have = cp?.employees_have || 0;
  let need = cp?.employees_needed || 0;
  (jobs || []).forEach((j) => { have += j.employees_have || 0; need += j.employees_needed || 0; });
  // Also count applications marked 'hired' as hired employees.
  const hired = all.filter((a) => a.status === "hired").length;
  if (hired > have) have = hired;
  animateCount("coHave", have, 900);
  animateCount("coNeed", need, 900);
  const hireBar = document.getElementById("coHireBar");
  if (hireBar) hireBar.style.setProperty("--w", (need > 0 ? Math.min(100, Math.round((have / need) * 100)) : 0) + "%");
...
/* =========================================================
   MINI PROFILE VIEWER — floating, draggable, closable panel
   Works for any user (student/admin) or company.
   ========================================================= */
function openMiniProfile(entity, kind) {
  const panel = document.getElementById("miniProfile");
  const body = document.getElementById("miniProfileBody");
  if (!panel || !body) return;

  const isCompany = kind === "company";
  const name = isCompany ? (entity.org_name || "Company") : (entity.full_name || entity.email || "User");
  const sub = isCompany
    ? (entity.industry || "Recruiter")
    : (entity.role === "admin" ? "Placement Cell Admin" : (entity.branch || "Student"));
  const bio = isCompany
    ? (entity.about_us || "No description provided.")
    : (entity.email || "");
  const avatarUrl = entity.avatar_url || entity.banner_url || "";
  const initials = name.slice(0, 2).toUpperCase();
  const skills = !isCompany && Array.isArray(entity.skills) ? entity.skills.slice(0, 12) : [];
  ...
  const details = isCompany
    ...
    : [
        ...
        ["CGPA", entity.cgpa],
        ["Profile completion", entity.profile_completion != null ? entity.profile_completion + "%" : null],
      ].filter(([, v]) => v);

  body.innerHTML = `
    <div class="mp-banner" style="${entity.banner_url ? `background-image:url(${entity.banner_url})` : ""}"></div>
    <div class="mp-avatar-wrap">
      <div class="mp-avatar" style="${avatarUrl ? `background-image:url(${avatarUrl})` : ""}">${avatarUrl ? "" : initials}</div>
    </div>
    <div class="mp-info">
      <h3 class="mp-name">${escapeHtml(name)}</h3>
      <p class="mp-sub">${escapeHtml(sub)}</p>
      ${bio ? `<p class="mp-bio">${escapeHtml(bio)}</p>` : ""}
      ${details.length ? `<div class="mp-details">${details.map(([k, v]) => `<div class="mp-detail-row"><span class="k">${k}</span><span class="v">${escapeHtml(String(v))}</span></div>`).join("")}</div>` : ""}
      ${skills.length ? `<div class="mp-skills">${skills.map((s) => `<span class="mp-skill">${escapeHtml(s)}</span>`).join("")}</div>` : ""}
      <div class="mp-actions">
        <button class="btn-primary btn-sm" id="mpMsgBtn">Message</button>
        ${isCompany ? `<button class="btn-ghost btn-sm" id="mpFullBtn">Full Profile</button>` : ""}
      </div>
      ${!isCompany && entity.id !== currentUser?.id ? `<div id="mpFriendBtnWrap" class="mp-actions" style="margin-top:0.5rem"></div>` : ""}
    </div>`;

  panel.hidden = false;
  requestAnimationFrame(() => panel.classList.add("show"));

  if (!isCompany && entity.id !== currentUser?.id) {
    updateMiniProfileFriendButton(entity.id);
  }

  const msgBtn = document.getElementById("mpMsgBtn");
  if (msgBtn) msgBtn.addEventListener("click", () => {
    closeMiniProfile();
    openConversationWith(entity.id, name);
  });
  const fullBtn = document.getElementById("mpFullBtn");
  if (fullBtn) fullBtn.addEventListener("click", () => {
    closeMiniProfile();
    openCompanyPublic(entity);
  });
}

async function updateMiniProfileFriendButton(otherId) {
  const wrap = document.getElementById("mpFriendBtnWrap");
  if (!wrap) return;
  wrap.innerHTML = `<button class="btn-ghost btn-sm" disabled style="opacity:0.6">Checking…</button>`;
  try {
    const { data, error } = await supabase
      .from("friends")
      .select("id, status, requester_id")
      .or(`and(requester_id.eq.${currentUser.id},recipient_id.eq.${otherId}),and(requester_id.eq.${otherId},recipient_id.eq.${currentUser.id})`)
      .maybeSingle();
    if (error) throw error;

    if (!data) {
      wrap.innerHTML = `<button class="btn-primary btn-sm" id="mpAddFriendBtn">Add Friend</button>`;
      const btn = document.getElementById("mpAddFriendBtn");
      if (btn) btn.addEventListener("click", async () => {
        btn.disabled = true;
        btn.textContent = "Sending…";
        await sendFriendRequest(otherId);
        updateMiniProfileFriendButton(otherId);
      });
    } else if (data.status === "accepted") {
      wrap.innerHTML = `<button class="btn-ghost btn-sm" disabled style="opacity:0.7">✓ Friends</button>`;
    } else if (data.status === "pending" && data.requester_id === currentUser.id) {
      wrap.innerHTML = `<button class="btn-ghost btn-sm" disabled style="opacity:0.7">Request Sent</button>`;
    } else if (data.status === "pending" && data.requester_id === otherId) {
      wrap.innerHTML = `<button class="btn-primary btn-sm" id="mpAcceptFriendBtn">Accept Request</button><button class="btn-ghost btn-sm" id="mpDeclineFriendBtn" style="margin-left:0.5rem">Decline</button>`;
      const acceptBtn = document.getElementById("mpAcceptFriendBtn");
      if (acceptBtn) acceptBtn.addEventListener("click", async () => {
        acceptBtn.disabled = true;
        await acceptFriend(data.id);
        updateMiniProfileFriendButton(otherId);
      });
      const declineBtn = document.getElementById("mpDeclineFriendBtn");
      if (declineBtn) declineBtn.addEventListener("click", async () => {
        declineBtn.disabled = true;
        await declineFriend(data.id);
        updateMiniProfileFriendButton(otherId);
      });
    } else {
      wrap.innerHTML = `<button class="btn-primary btn-sm" id="mpAddFriendBtn">Add Friend</button>`;
    ...
    console.error("updateMiniProfileFriendButton", err);
    wrap.innerHTML = `<button class="btn-primary btn-sm" id="mpAddFriendBtn">Add Friend</button>`;
    const btn = document.getElementById("mpAddFriendBtn");
    if (btn) btn.addEventListener("click", async () => {
      btn.disabled = true;
...
function initMiniProfileDrag() {
  const panel = document.getElementById("miniProfile");
  const head = document.getElementById("miniProfileHead");
  const closeBtn = document.getElementById("miniProfileClose");
  if (!panel || !head) return;
  if (closeBtn) closeBtn.addEventListener("click", closeMiniProfile);
...
async function loadApplicants() {
  ...
  body.innerHTML = allApplicants.map((a) => `
    <tr>
      ...
      <td class="applicant-actions">
        ...
        <button class="row-btn" data-call-interview="${a.id}">Call for Interview</button>
      </td>
    </tr>`).join("");
  ...
  body.querySelectorAll("[data-call-interview]").forEach((b) => b.addEventListener("click", () => {
    const a = allApplicants.find((x) => x.id === b.dataset.callInterview);
    if (a) startCall(a.student_id, "interview");
  }));
}
...
async function loadConversations() {
  ...
  if (otherIds.length) {
    const { data: profs } = await supabase.from("profiles").select("id, full_name, email, avatar_url, role, branch, skills, cgpa, profile_completion").in("id", otherIds);
    (profs || []).forEach((p) => { conversationProfiles[p.id] = p; });
    // Company accounts live in company_profiles, not profiles — fetch any ids not yet resolved
    const missingIds = otherIds.filter((id) => !conversationProfiles[id]);
    if (missingIds.length) {
      const { data: comps } = await supabase.from("company_profiles").select("*").in("id", missingIds);
      (comps || []).forEach((c) => { conversationProfiles[c.id] = { ...c, role: "company", _company: true }; });
    }
  }
  ...
  el.innerHTML = allConversations.map((c) => {
    ...
    const name = other?.full_name || other?.org_name || other?.email || "User";
    ...
    return `
    <div class="conv-item" data-id="${c.id}" data-name="${name.toLowerCase()}" data-other="${otherId}">
      <div class="${avatarClass}" style="${avatarStyle}" data-open-profile="${otherId}">${other?.avatar_url ? "" : name.slice(0, 2).toUpperCase()}</div>
  ...
  el.querySelectorAll(".conv-item").forEach((item) => item.addEventListener("click", (e) => {
    if (e.target.dataset.openProfile) { e.stopPropagation(); openMiniProfileFor(e.target.dataset.openProfile); return; }
    const c = allConversations.find((x) => x.id === item.dataset.id);
    const otherId = c.user_a === currentUser.id ? c.user_b : c.user_a;
    openConversation(c, conversationProfiles[otherId]);
  }));
...
async function openConversation(conv, other) {
  currentConversation = conv;
  const otherId = conv.user_a === currentUser.id ? conv.user_b : conv.user_a;
  const profile = other && typeof other === "object" ? other : conversationProfiles[otherId];
  const name = profile?.full_name || profile?.org_name || profile?.email || (typeof other === "string" ? other : "User");
  ...
  document.getElementById("convWhoName").textContent = name;
  const avEl = document.getElementById("convWhoAvatar");
  if (avEl) {
    avEl.className = profile?.avatar_url ? "conv-avatar has-img" : "conv-avatar";
    avEl.style.cssText = profile?.avatar_url ? `background-image:url(${profile.avatar_url})` : "";
    avEl.textContent = profile?.avatar_url ? "" : name.slice(0, 2).toUpperCase();
    avEl.style.cursor = "pointer";
    avEl.onclick = () => openMiniProfileFor(otherId);
  }
...
function openMiniProfileFor(otherId) {
  const profile = conversationProfiles[otherId];
  if (!profile) { showToast("Couldn't load this profile.", "error"); return; }
  openMiniProfile(profile, profile._company ? "company" : "user");
}
...
async function loadMessages(conversationId) {
  ...
  if (!el) return;
  const otherId = currentConversation && (currentConversation.user_a === currentUser.id ? currentConversation.user_b : currentConversation.user_a);
  const other = conversationProfiles[otherId] || {};
  const me = currentProfile || {};
  el.innerHTML = (data || []).map((m) => {
    const isMine = m.sender_id === currentUser.id;
    const sender = isMine ? me : other;
    const name = sender?.full_name || sender?.org_name || sender?.email || "User";
    const avatarStyle = sender?.avatar_url ? `background-image:url(${sender.avatar_url})` : "";
    const avatarClass = sender?.avatar_url ? "msg-avatar has-img" : "msg-avatar";
    const openAttr = isMine ? "" : `data-open-profile="${otherId}"`;
    return `
    <div class="msg-row ${isMine ? "mine" : "theirs"}">
      <div class="${avatarClass}" style="${avatarStyle}" ${openAttr}>${sender?.avatar_url ? "" : name.slice(0, 2).toUpperCase()}</div>
  ...
  el.querySelectorAll("[data-open-profile]").forEach((a) => a.addEventListener("click", () => openMiniProfileFor(a.dataset.openProfile)));
...
async function loadDbNotifications() {
  ...
  const iconFor = { application: "📥", message: "💬", status: "🔔", friend_request: "👤", friend_accepted: "🎉" };
  return (data || []).map((n) => ({
    id: n.id,
    icon: iconFor[n.type] || "🔔",
    title: n.title,
...
/* =========================================================
   FRIENDS + VOICE CALLING (additive module)
   Tables (existing, managed by the user):
     friends(id, requester_id, recipient_id, status, created_at, updated_at)
     calls(id, caller_id, callee_id, call_type, room_name, status, created_at, updated_at)
   RPC: search_students(q text) -> (id, full_name, email, avatar_url)
   ========================================================= */
...
function initFriends() {
  ...
  if (searchInput) {
    ...
    searchInput.addEventListener("input", () => {
      clearTimeout(t);
      t = setTimeout(() => searchStudents(searchInput.value.trim()), 300);
    });
  }
  ...
  if (endBtn) endBtn.addEventListener("click", leaveActiveCall);
  const cancelBtn = document.getElementById("outgoingCallCancelBtn");
  if (cancelBtn) cancelBtn.addEventListener("click", cancelOutgoingCall);
  const cameraBtn = document.getElementById("callCameraToggleBtn");
  if (cameraBtn) cameraBtn.addEventListener("click", toggleLocalCamera);
  const muteBtn = document.getElementById("callMuteBtn");
  if (muteBtn) muteBtn.addEventListener("click", toggleMute);
  const holdBtn = document.getElementById("callHoldBtn");
  if (holdBtn) holdBtn.addEventListener("click", toggleHold);
  const streamBtn = document.getElementById("callStreamBtn");
  if (streamBtn) streamBtn.addEventListener("click", toggleScreenShare);
}
...
async function searchStudents(q) {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      el.innerHTML = `<p class="muted" style="padding:0.5rem">No students found.</p>`;
      return;
    }
    el.innerHTML = data
      ...
      .map((s) => {
        ...
        const av = s.avatar_url
          ? `<div class="friend-avatar" style="background-image:url('${s.avatar_url}')"></div>`
          : `<div class="friend-avatar">${initials}</div>`;
        return `<div class="friend-row">
          ${av}
          <div class="friend-row-body">
            <div class="friend-row-name">${escapeHtml(s.full_name || "Unnamed")}</div>
            <div class="friend-row-sub">${escapeHtml(s.email || "")}</div>
          </div>
          <div class="friend-row-actions">
            <button class="btn-primary btn-sm" data-add-friend="${s.id}">Add Friend</button>
          </div>
        </div>`;
      })
      .join("");
    el.querySelectorAll("[data-add-friend]").forEach((b) =>
      b.addEventListener("click", () => sendFriendRequest(b.dataset.addFriend))
    );
  } catch (err) {
    console.error("searchStudents", err);
    el.innerHTML = `<p class="muted" style="padding:0.5rem">Search failed.</p>`;
  }
}
...
async function sendFriendRequest(recipientId) {
  try {
    const { data: existing } = await supabase
      ...
      .maybeSingle();
    if (existing) {
      if (existing.status === "accepted") showToast("You are already friends.", "info");
      else if (existing.status === "pending" && existing.requester_id === currentUser.id) showToast("Request already sent.", "info");
      else if (existing.status === "pending") showToast("This person already sent you a request — check your incoming requests.", "info");
      else showToast("A previous request was declined. Try again.", "info");
      return;
    }
    ...
    await supabase.from("notifications").insert({
      ...
      body: "Tap to view and accept in the Friends tab.",
      link_view: "friends",
    });
    ...
    if (err?.code === "23505") showToast("Request already sent.", "info");
    else showToast("Could not send request.", "error");
  }
}

async function loadFriendsView() {
  await Promise.all([loadIncomingRequests(), loadOutgoingRequests(), loadAcceptedFriends()]);
  subscribeFriends();
  subscribeCalls();
}
...
async function loadIncomingRequests() {
  ...
  try {
    const { data, error } = await supabase
      .from("friends")
      .select("id, requester_id, status, created_at, profiles!friends_requester_id_fkey(full_name, email, avatar_url)")
    ...
    if (!data || data.length === 0) {
      el.innerHTML = `<p class="muted" style="padding:0.5rem">No incoming requests.</p>`;
      return;
    }
    el.innerHTML = data
      .map((r) => {
        ...
        const av = p.avatar_url
          ? `<div class="friend-avatar" style="background-image:url('${p.avatar_url}')"></div>`
          : `<div class="friend-avatar">${initials}</div>`;
        return `<div class="pending-item">
          ${av}
          <div class="friend-row-body"><div class="friend-row-name">${escapeHtml(name)}</div></div>
          <div class="friend-row-actions">
            <button class="btn-primary btn-sm" data-accept-friend="${r.id}">Accept</button>
            <button class="btn-ghost btn-sm" data-decline-friend="${r.id}">Decline</button>
          </div>
        </div>`;
      })
      .join("");
    el.querySelectorAll("[data-accept-friend]").forEach((b) =>
      b.addEventListener("click", () => acceptFriend(b.dataset.acceptFriend))
    );
    el.querySelectorAll("[data-decline-friend]").forEach((b) =>
      b.addEventListener("click", () => declineFriend(b.dataset.declineFriend))
    );
  } catch (err) {
    console.error("loadIncomingRequests", err);
    el.innerHTML = `<p class="muted" style="padding:0.5rem">Could not load requests.</p>`;
  }
}

async function loadOutgoingRequests() {
  const el = document.getElementById("outgoingRequests");
  if (!el) return;
  try {
    const { data, error } = await supabase
      .from("friends")
      .select("id, recipient_id, status, profiles!friends_recipient_id_fkey(full_name, email, avatar_url)")
      .eq("requester_id", currentUser.id)
      .eq("status", "pending");
    if (error) throw error;
    if (!data || data.length === 0) {
      el.innerHTML = `<p class="muted" style="padding:0.5rem">No outgoing requests.</p>`;
      return;
    }
    el.innerHTML = data
      .map((r) => {
        ...
        return `<div class="pending-item">
          ...
          <div class="friend-row-body"><div class="friend-row-name">${escapeHtml(name)}</div></div>
          <div class="friend-row-actions">
            <button class="btn-ghost btn-sm" data-cancel-friend="${r.id}">Cancel</button>
          </div>
        </div>`;
      })
      .join("");
    el.querySelectorAll("[data-cancel-friend]").forEach((b) =>
      b.addEventListener("click", () => cancelFriendRequest(b.dataset.cancelFriend))
    );
  } catch (err) {
    console.error("loadOutgoingRequests", err);
    el.innerHTML = `<p class="muted" style="padding:0.5rem">Could not load requests.</p>`;
  }
}
...
async function loadAcceptedFriends() {
  ...
  try {
    const { data: incoming, error: e1 } = await supabase
      .from("friends")
      .select("id, requester_id, profiles!friends_requester_id_fkey(full_name, email, avatar_url)")
      .eq("recipient_id", currentUser.id)
      .eq("status", "accepted");
    if (e1) throw e1;
    const { data: outgoing, error: e2 } = await supabase
      .from("friends")
      .select("id, recipient_id, profiles!friends_recipient_id_fkey(full_name, email, avatar_url)")
      .eq("requester_id", currentUser.id)
      .eq("status", "accepted");
    if (e2) throw e2;
    const friends = [
      ...(incoming || []).map((r) => ({ id: r.requester_id, rowId: r.id, ...r.profiles })),
      ...(outgoing || []).map((r) => ({ id: r.recipient_id, rowId: r.id, ...r.profiles })),
    ];
    const countEl = document.getElementById("friendsCount");
    if (countEl) countEl.textContent = friends.length;
    if (friends.length === 0) {
      el.innerHTML = `<p class="muted" style="padding:0.5rem">You haven't added any friends yet. Search above to find students.</p>`;
      return;
    }
    el.innerHTML = friends
      .map((f) => {
        ...
        const av = f.avatar_url
          ? `<div class="friend-avatar" style="background-image:url('${f.avatar_url}')"></div>`
          : `<div class="friend-avatar">${initials}</div>`;
        return `<div class="friend-row">
          ${av}
          <div class="friend-row-body">
            <div class="friend-row-name">${escapeHtml(name)}</div>
          ...
          <div class="friend-row-actions">
            ...
            <button class="btn-danger btn-sm" data-remove-friend="${f.rowId}" data-remove-name="${escapeHtml(name)}" title="Remove friend">
              <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:4px"><path d="M18 6 6 18M6 6l12 12"/></svg>Remove
            </button>
          </div>
        </div>`;
      })
    ...
    el.querySelectorAll("[data-remove-friend]").forEach((b) =>
      b.addEventListener("click", () => removeFriend(b.dataset.removeFriend, b.dataset.removeName))
    );
  } catch (err) {
    console.error("loadAcceptedFriends", err);
    el.innerHTML = `<p class="muted" style="padding:0.5rem">Could not load your friends.</p>`;
  }
}

async function removeFriend(friendRowId, name) {
  if (!friendRowId) return;
  const ok = window.confirm(`Remove ${name || "this friend"} from your friends list?`);
  if (!ok) return;
  try {
...
async function acceptFriend(friendRowId) {
  try {
...
async function declineFriend(friendRowId) {
  try {
    const { error } = await supabase.from("friends").delete().eq("id", friendRowId);
    if (error) throw error;
...
async function cancelFriendRequest(friendRowId) {
  try {
    const { error } = await supabase.from("friends").delete().eq("id", friendRowId);
    if (error) throw error;
...
function subscribeFriends() {
  if (friendsChannel) { try { supabase.removeChannel(friendsChannel); } catch (e) {} }
  friendsChannel = supabase
    .channel("friends-realtime")
    .on(
      "postgres_changes",
      { event: "INSERT", schema: "public", table: "friends", filter: `recipient_id=eq.${currentUser.id}` },
      () => { loadIncomingRequests(); loadAcceptedFriends(); renderNotifications(); }
    )
    .on(
      "postgres_changes",
      { event: "INSERT", schema: "public", table: "friends", filter: `requester_id=eq.${currentUser.id}` },
      () => { loadOutgoingRequests(); loadAcceptedFriends(); }
    )
    .on(
      "postgres_changes",
      { event: "UPDATE", schema: "public", table: "friends", filter: `requester_id=eq.${currentUser.id}` },
      () => { loadOutgoingRequests(); loadAcceptedFriends(); renderNotifications(); }
    )
    .on(
      "postgres_changes",
      { event: "UPDATE", schema: "public", table: "friends", filter: `recipient_id=eq.${currentUser.id}` },
      () => { loadIncomingRequests(); loadAcceptedFriends(); }
    )
    .subscribe();
}
...
const CALL_RING_TIMEOUT_MS = 45000; // how long we let a call ring before treating it as "no answer"
let outgoingRingTimeout = null;
...
async function startCall(calleeId, callType) {
  ...
  try {
    ...
    // Show a persistent "ringing" screen to the caller instead of a toast
    // that disappears in a few seconds — this is the call window that was
    // missing before.
    await showOutgoingCall(data.id, calleeId, callType);
    // Prepare the peer connection + signaling channel right away so that
    // when the callee accepts, the offer can go out instantly — instead of
    // waiting for the "accepted" database event → subscribe → presence
    // round-trip, which was the main source of "took too long to connect".
    await prepareCall(data.id, true);
  } catch (err) {
    console.error("startCall", err);
    showToast("Could not start the call. You may not be allowed to call this person.", "error");
  }
}

async function showOutgoingCall(callId, calleeId, callType) {
  ...
  const { data: callee } = await supabase
    ...
    .eq("id", calleeId)
    .maybeSingle();
  const name = callee?.full_name || "Someone";
  if (nameEl) nameEl.textContent = name;
  if (statusEl) statusEl.textContent = callType === "interview" ? "Calling for an interview… Ringing" : "Ringing…";
  if (avEl) {
    if (callee?.avatar_url) {
      avEl.style.backgroundImage = `url('${callee.avatar_url}')`;
      avEl.textContent = "";
    } else {
  ...
  clearTimeout(outgoingRingTimeout);
  outgoingRingTimeout = setTimeout(() => handleNoAnswer(callId), CALL_RING_TIMEOUT_MS);
}
...
function hideOutgoingCall() {
  ...
  clearTimeout(outgoingRingTimeout);
  outgoingRingTimeout = null;
}

async function handleNoAnswer(callId) {
  // Only fires if the call is still ringing when the timeout elapses.
  ...
  teardownCall();
  try {
    // Prefer a dedicated "missed" status; fall back to "ended" if the
    // database doesn't have that value allowed.
    ...
    console.error("handleNoAnswer", err);
  }
}
...
async function cancelOutgoingCall() {
  ...
  try {
    ...
    console.error("cancelOutgoingCall", err);
  }
}

function subscribeCallStatus(callId, isCaller) {
  if (callStatusChannel) { try { supabase.removeChannel(callStatusChannel); } catch (e) {} callStatusChannel = null; }
  callStatusChannel = supabase
    ...
    .on(
      ...
      (payload) => {
        ...
        if (!st) return;
        if (st === "accepted") {
          // Caller: callee picked up — reveal the call view. (The peer
          // connection was already prepared during ringing, and the presence
          // handler may have already revealed it; this is a backup.) Callee
          // ignores it — they already know they accepted.
          if (isCaller) { hideOutgoingCall(); showCallOverlay(); }
        } else if (st === "declined") {
          if (isCaller) {
            hideOutgoingCall();
          ...
          // The other side ended the call (or it timed out). Tear down local
          // media + UI. Don't call endCall here — the side that hung up
          // already wrote "ended"; calling it again would just echo.
          hideOutgoingCall();
          if (st === "missed") showToast("No answer.", "info");
          teardownCall();
        }
      }
    )
...
function subscribeCalls() {
  if (callsChannel) { try { supabase.removeChannel(callsChannel); } catch (e) {} }
  callsChannel = supabase
    ...
    .on(
      ...
      (payload) => {
        if (payload.new?.status === "ringing") showIncomingCall(payload.new);
      }
    )
    .subscribe();
}
...
async function showIncomingCall(callRow) {
  ...
  const { data: caller } = await supabase
    ...
    .maybeSingle();
  const name = caller?.full_name || "Someone";
  const label = callRow.call_type === "interview" ? "is calling for an interview…" : "is calling…";
  document.getElementById("callCallerName").textContent = name;
  document.getElementById("callTypeLabel").textContent = label;
  const avEl = document.getElementById("callCallerAvatar");
  if (caller?.avatar_url) {
    avEl.style.backgroundImage = `url('${caller.avatar_url}')`;
    avEl.textContent = "";
  } else {
    avEl.style.backgroundImage = "";
    avEl.textContent = name.charAt(0).toUpperCase();
  ...
  incomingRingTimeout = setTimeout(async () => {
    if (incomingCallId !== callRow.id) return;
    document.getElementById("incomingCallOverlay").classList.remove("show");
    const missedId = incomingCallId;
    incomingCallId = null;
    activeCallId = null;
    try {
      let { error } = await supabase
      ...
      console.error("incoming call timeout", err);
    }
...
async function acceptIncomingCall() {
  ...
  try {
    ...
    await prepareCall(incomingCallId, false);
    showCallOverlay();
  } catch (err) {
...
async function declineIncomingCall() {
  ...
  try {
    ...
    console.error("declineIncomingCall", err);
  }
  document.getElementById("incomingCallOverlay").classList.remove("show");
  incomingCallId = null;
  activeCallId = null;
...
/* ---------- WebRTC (peer-to-peer, no server, no login) ----------
   Signaling (offer/answer/ICE candidates) rides on a Supabase Realtime
   channel scoped to the call id — no third-party call service needed.
   Media flows directly between the two browsers (peer-to-peer). */

let rtcPeer = null;
let localMediaStream = null;
let signalChannel = null;
let callStatusChannel = null;
let offerAlreadySent = false;
let answerReceived = false;
let pendingIceCandidates = []; // candidates that arrive before the remote description is set
let connectTimeoutHandle = null;
let offerRetryHandle = null;

const FALLBACK_ICE_SERVERS = [
  { urls: "stun:stun.l.google.com:19302" },
  { urls: "stun:stun1.l.google.com:19302" },
  // STUN-only. Used only if /api/turn-credentials isn't configured or the
  // request fails — same-network calls still work, but calls between two
  // people on strict NATs/firewalls (mobile data, campus/corporate wifi)
  // will not connect without a TURN relay. See getIceServers() below.
];

// Short-lived TURN credentials are fetched fresh for every call instead of
// hardcoded, because real TURN credentials expire and must never be
// long-lived secrets sitting in client-side JS. If the endpoint isn't set up
// yet (no METERED_APP_DOMAIN/METERED_API_KEY env vars — see
// api/turn-credentials.js), this quietly
// falls back to STUN-only so the app still works for same-network calls.
async function getIceServers() {
  try {
    ...
    if (data?.ok && Array.isArray(data.iceServers) && data.iceServers.length) {
      // Keep Google STUN alongside the TURN servers Cloudflare returns —
      // more candidate paths generally means a faster connection.
      return [...FALLBACK_ICE_SERVERS, ...data.iceServers];
    }
    return FALLBACK_ICE_SERVERS;
  } catch (err) {
    console.warn("getIceServers: falling back to STUN-only", err);
    return FALLBACK_ICE_SERVERS;
  }
}

async function prepareCall(callId, isCaller) {
  const container = document.getElementById("callFrameContainer");
  const localVideo = document.getElementById("localVideo");
  const remoteVideo = document.getElementById("remoteVideo");
  if (!container || !localVideo || !remoteVideo) {
    showToast("Call UI not available.", "error");
    return;
  }
  ...
  clearTimeout(offerRetryHandle);

  try {
    // Camera is NOT required to join a call. We request audio first (mic is the
    // minimum needed for a voice call), then opportunistically add the camera
    // in a separate request. If the user has no camera or denies camera
    // permission, the call still connects as audio-only — the browser never
    // blocks joining on a camera prompt.
    let videoStream = null;
    try {
      localMediaStream = await navigator.mediaDevices.getUserMedia({ video: false, audio: true });
    } catch (audioErr) {
      console.error("Microphone unavailable", audioErr);
      showToast("Microphone is required to join a call.", "error");
      throw audioErr;
    }
    // Try to add the camera without blocking the call. If the user denies or
    // has no camera, we simply continue audio-only.
    try {
      videoStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
      videoStream.getTracks().forEach((t) => localMediaStream.addTrack(t));
    } catch (videoErr) {
      console.warn("Camera unavailable — continuing audio-only", videoErr);
    }
    ...
    localMediaStream.getTracks().forEach((track) => rtcPeer.addTrack(track, localMediaStream));

    const titleEl = document.getElementById("callOverlayTitle");

    rtcPeer.ontrack = (e) => {
      if (remoteVideo.srcObject !== e.streams[0]) remoteVideo.srcObject = e.streams[0];
      const ph = document.getElementById("callStagePlaceholder");
    ...
    rtcPeer.onconnectionstatechange = () => {
      ...
      const dot = document.getElementById("callHudDot");
      if (s === "connected") {
        if (titleEl) titleEl.textContent = "In call";
        if (dot) { dot.classList.add("connected"); dot.classList.remove("hold"); }
        clearTimeout(connectTimeoutHandle);
        startCallTimer();
      } else if (s === "failed") {
    ...
    // `private: true` is required: Supabase Realtime now enforces
    // Authorization on Broadcast/Presence channels by default. Without this
    // flag (and the matching RLS policies on realtime.messages — see the
    // 20260729_call_signaling_realtime_auth migration), the offer/answer/ICE
    // messages below are silently dropped by the server, which is why calls
    // used to hang on "Connecting…" forever with no audio or video.
    signalChannel = supabase.channel(`call-${callId}`, {
    ...
    rtcPeer.onicecandidate = (e) => {
      if (e.candidate) {
        signalChannel.send({
          type: "broadcast",
          event: "signal",
          payload: { kind: "ice-candidate", from: currentUser.id, candidate: e.candidate },
        });
      }
    };
    ...
    signalChannel
      .on("broadcast", { event: "signal" }, async ({ payload }) => {
        if (!payload || payload.from === currentUser.id || !rtcPeer) return;
        try {
          if (payload.kind === "ready") {
            // Callee is on the channel and listening — safe to send the
            // offer now. This is the primary trigger; presence sync is a
            // backup below.
            if (isCaller && !offerAlreadySent && rtcPeer) {
              await sendOffer(callId);
            }
          } else if (payload.kind === "offer") {
            // Callee: process the offer. Guard against duplicates from
            // caller retries — if we already have a remote description,
            // we already answered; just resend the same answer.
            if (rtcPeer.currentRemoteDescription) {
              // Already answered — resend the answer in case the caller
              // missed it (that's why they retried the offer).
              const ans = rtcPeer.localDescription;
              if (ans) {
                signalChannel.send({
                  type: "broadcast",
                  event: "signal",
                  payload: { kind: "answer", from: currentUser.id, sdp: ans },
                });
              }
              return;
            }
            startConnectTimer(callId, isCaller);
            await rtcPeer.setRemoteDescription(new RTCSessionDescription(payload.sdp));
            ...
            signalChannel.send({
              type: "broadcast",
              event: "signal",
              payload: { kind: "answer", from: currentUser.id, sdp: answer },
            });
            ...
            // If the offer/answer hasn't been applied yet, addIceCandidate
            // throws and the candidate is lost — queue it instead and add
            // it once a remote description exists. Without this, candidates
            // that race ahead of the offer get silently dropped, which can
            // be enough to prevent the connection from ever completing.
            ...
            // The other side hung up. Tear down locally — they already
            // marked the call "ended" in the database, so we don't endCall
            // again (that would echo and can race with their write).
            teardownCall();
          }
        } catch (err) {
      ...
      .on("presence", { event: "sync" }, async () => {
        // Backup trigger: if the callee joins and we somehow missed their
        // "ready" broadcast, presence sync will also kick off the offer.
        if (!isCaller || offerAlreadySent || !rtcPeer) return;
        const others = Object.keys(signalChannel.presenceState()).filter((k) => k !== currentUser.id);
        if (others.length > 0) {
          hideOutgoingCall();
          showCallOverlay();
          await sendOffer(callId);
      ...
      .subscribe(async (status) => {
        if (status === "SUBSCRIBED") {
          await signalChannel.track({ online_at: new Date().toISOString() });
          // Callee: announce readiness so the caller knows it's safe to
          // send the offer. Send twice for redundancy — broadcast messages
          // are fire-and-forget and can occasionally be lost.
          if (!isCaller) {
            for (let i = 0; i < 2; i++) {
              signalChannel.send({
                type: "broadcast",
                event: "signal",
                payload: { kind: "ready", from: currentUser.id },
              });
    ...
    // Subscribe to call-status changes (declined / ended / missed / accepted)
    // so BOTH sides tear down when the other hangs up. Previously only the
    // caller subscribed, so when the caller left, the callee stayed "in
    // call" indefinitely.
    subscribeCallStatus(callId, isCaller);
  } catch (err) {
    console.error("prepareCall", err);
    const reason =
      err?.message || (typeof err === "string" ? err : null) || "Unknown error";
    showToast("Could not join the call: " + reason, "error");
    teardownCall();
    await endCall(callId);
  }
}

// Caller: create the offer, set local description, send it, and start a
// retry loop in case the offer or the callee's answer is lost in transit.
async function sendOffer(callId) {
  if (!rtcPeer || offerAlreadySent) return;
  offerAlreadySent = true;
  hideOutgoingCall();
  showCallOverlay();
  startConnectTimer(callId, true);
  try {
    ...
    signalChannel.send({
      type: "broadcast",
      event: "signal",
      payload: { kind: "offer", from: currentUser.id, sdp: offer },
    });
    // Retry: if no answer arrives within 4s, resend the offer. Broadcast
    // messages can be dropped, and without a retry the call silently hangs.
    let attempts = 0;
    offerRetryHandle = setTimeout(function retry() {
      if (answerReceived || !rtcPeer || rtcPeer.connectionState === "connected") return;
      if (attempts >= 3) return;
      attempts++;
      signalChannel.send({
        type: "broadcast",
        event: "signal",
        payload: { kind: "offer", from: currentUser.id, sdp: rtcPeer.localDescription },
      });
      offerRetryHandle = setTimeout(retry, 4000);
    }, 4000);
  } catch (err) {
...
// ICE can get stuck in "checking" forever (no failure event fires) if NAT
// traversal never succeeds. The timer starts only once signaling actually
// begins (offer sent or received) — not when the call is placed — so the
// 15s grace window isn't burned while the phone is still ringing.
function startConnectTimer(callId, isCaller) {
  clearTimeout(connectTimeoutHandle);
  let triedIceRestart = false;
  connectTimeoutHandle = setTimeout(async () => {
    if (!rtcPeer || rtcPeer.connectionState === "connected") return;
    if (isCaller && !triedIceRestart) {
      triedIceRestart = true;
      try {
        const offer = await rtcPeer.createOffer({ iceRestart: true });
        await rtcPeer.setLocalDescription(offer);
        signalChannel.send({
          type: "broadcast",
          event: "signal",
      ...
      connectTimeoutHandle = setTimeout(() => {
        if (rtcPeer && rtcPeer.connectionState !== "connected") {
          showToast("Couldn't connect the call — check your network and try again.", "error");
          endActiveCall(callId);
        }
      }, 12000);
    } else {
  ...
  if (signalChannel) {
    try {
      signalChannel.send({
        type: "broadcast",
        event: "signal",
        payload: { kind: "bye", from: currentUser.id },
      });
    } catch (e) {}
  }
}

// Used by both the Leave button and a failed connection: tell the other
// side to hang up immediately (no waiting for the DB event), tear down
// local media, then mark the call ended.
...
async function flushPendingIceCandidates() {
  if (!rtcPeer || pendingIceCandidates.length === 0) return;
...
function toggleLocalCamera() {
  ...
  if (!videoTrack) {
    showToast("No camera available on this call.", "info");
    return;
  }
  videoTrack.enabled = !videoTrack.enabled;
  updateCameraToggleUI();
  renegotiateVideoSender();
}

async function renegotiateVideoSender() {
  if (!rtcPeer) return;
  const videoTrack = localMediaStream ? localMediaStream.getVideoTracks()[0] : null;
  const videoSender = rtcPeer.getSenders().find((s) => s.track && s.track.kind === "video");
  if (videoTrack && videoTrack.enabled && videoSender) {
    try { await videoSender.replaceTrack(videoTrack); } catch (e) { console.warn("replaceTrack video", e); }
  } else if (videoTrack && !videoTrack.enabled && videoSender) {
    try { await videoSender.replaceTrack(null); } catch (e) { console.warn("replaceTrack null", e); }
  }
}

function updateCameraToggleUI() {
  const btn = document.getElementById("callCameraToggleBtn");
  const offLabel = document.getElementById("localVideoOff");
  const videoTrack = localMediaStream ? localMediaStream.getVideoTracks()[0] : null;
  const on = !!videoTrack && videoTrack.enabled;
  if (btn) {
    if (!videoTrack) {
      btn.classList.add("disabled", "camera-off");
      btn.setAttribute("disabled", "true");
    } else {
      btn.removeAttribute("disabled");
      btn.classList.remove("disabled", "camera-off");
      btn.classList.toggle("camera-off", !on);
    }
    btn.title = on ? "Turn camera off" : "Turn camera on";
  }
  if (offLabel) offLabel.hidden = on;
}
...
function toggleMute() {
  ...
  if (!audioTrack) {
    showToast("No microphone available on this call.", "info");
    return;
  }
...
function updateMuteToggleUI() {
  ...
  const audioTrack = localMediaStream ? localMediaStream.getAudioTracks()[0] : null;
...
let callHoldState = false;
function toggleHold() {
  if (!localMediaStream) return;
  callHoldState = !callHoldState;
  localMediaStream.getTracks().forEach((t) => { t.enabled = !callHoldState; });
  const btn = document.getElementById("callHoldBtn");
  const stage = document.getElementById("callFrameContainer");
  const dot = document.getElementById("callHudDot");
  if (btn) {
    btn.classList.toggle("held", callHoldState);
    btn.title = callHoldState ? "Resume call" : "Hold call";
  }
  if (stage) stage.classList.toggle("held", callHoldState);
  if (dot) dot.classList.toggle("hold", callHoldState);
  if (!callHoldState) {
    updateMuteToggleUI();
    updateCameraToggleUI();
  }
...
async function toggleScreenShare() {
  if (!rtcPeer) return;
  const btn = document.getElementById("callStreamBtn");
  if (screenStream) {
    ...
    if (sender && videoTrack) {
      try { await sender.replaceTrack(videoTrack); } catch (e) { console.warn("restore camera", e); }
    }
    if (btn) { btn.classList.remove("streaming"); btn.title = "Share your screen"; }
    const localVideo = document.getElementById("localVideo");
  ...
  try {
    ...
    const sender = rtcPeer.getSenders().find((s) => s.track && s.track.kind === "video");
    if (sender) {
      try { await sender.replaceTrack(screenTrack); } catch (e) { console.warn("replace screen", e); }
    }
    screenTrack.onended = () => {
      screenStream = null;
      if (btn) { btn.classList.remove("streaming"); btn.title = "Share your screen"; }
      const restore = localMediaStream.getVideoTracks()[0];
      if (sender && restore) {
        try { sender.replaceTrack(restore); } catch (e) {}
      }
      const localVideo = document.getElementById("localVideo");
      if (localVideo) localVideo.srcObject = localMediaStream;
    };
    if (btn) { btn.classList.add("streaming"); btn.title = "Stop screen share"; }
    const localVideo = document.getElementById("localVideo");
...
function teardownCall() {
  ...
  if (callStatusChannel) {
    try { supabase.removeChannel(callStatusChannel); } catch (e) {}
