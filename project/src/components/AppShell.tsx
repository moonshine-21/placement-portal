      ...
      <div className="flex flex-1 flex-col lg:ml-64 min-w-0">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-[var(--border)] bg-[var(--bg-base)]/70 backdrop-blur-xl backdrop-saturate-150 px-4 py-3.5 md:px-6">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            {/* Profile button — a circular avatar with an online dot,
                Discord/Slack-style. Clicking it no longer jumps straight
                to the profile page; instead it opens a small popover
                (banner, avatar, name, email, branch, bio, skills) with
                its own "Edit profile" button that does the actual
                navigating — see the popover below. */}
            <div className="relative">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="relative flex h-10 w-10 flex-shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-[var(--border-strong)] bg-gradient-to-br from-[var(--accent)] to-[var(--accent-2)] text-xs font-bold text-white transition-all hover:border-[var(--accent)] hover:-translate-y-0.5 hover:shadow-lg hover:shadow-[var(--accent)]/20 active:translate-y-0 active:scale-95"
              ...
              >
                ...
                {/* A little "online" dot, purely decorative — just makes
                    the header avatar feel alive, same idea as the
                    notification badge next to it. */}
                <span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-[var(--bg-base)] bg-emerald-400" />
              </button>

              {activeMenu === 'profile' && (
                <div
                  className="dropdown-panel absolute right-0 top-12 z-50 w-80 overflow-hidden animate-slide-down"
                  onClick={(e) => e.stopPropagation()}
                >
                  {/* Banner strip — falls back to a soft accent gradient
                      when the person hasn't uploaded a cover image. */}
                  <div className="h-20 w-full overflow-hidden bg-gradient-to-br from-[var(--accent)]/35 via-[var(--accent-2)]/20 to-transparent">
                    {profile?.banner_url && (
                  ...
                  <div className="px-4 pb-4">
                    ...
                    <div className="-mt-8 flex items-end justify-between gap-3">
                      <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-2xl border-4 border-[var(--bg-elevated)] bg-gradient-to-br from-[var(--accent)] to-[var(--accent-2)] text-lg font-bold text-white shadow-lg">
                        {avatarUrl ? (
                          <img src={avatarUrl} alt="" className="h-full w-full object-cover" />
                        ) : (
                          <span>{initials}</span>
