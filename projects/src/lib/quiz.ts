...
export async function loadQuizAssignmentStatuses(quizId: string): Promise<Record<string, 'sent' | 'in_progress' | 'completed'>> {
  const { data } = await supabase.from('quiz_assignments').select('student_id, status').eq('quiz_id', quizId);
  const map: Record<string, 'sent' | 'in_progress' | 'completed'> = {};
  (data || []).forEach((r: { student_id: string; status: 'sent' | 'in_progress' | 'completed' }) => { map[r.student_id] = r.status; });
  return map;
}
