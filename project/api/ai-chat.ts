...
export default async function handler(req: Request): Promise<Response> {
  ...
  // noticeably faster than Gemini. Gemini is the fallback below for
  // whenever Groq is unset, rate-limited, or having a slow moment.
  if (groqKey) {
  ...
  if (geminiKey && Date.now() < overallDeadline) {
    const reply = await tryProvider(
