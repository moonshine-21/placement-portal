...
export function CallManager({ startCallRequest, onCallConsumed }: Props) {
  ...
  const remoteStreamRef = useRef<MediaStream | null>(null); // holds the remote MediaStream so we can re-attach after UI mounts
  ...
  const prepareCall = async (id: string, isCaller: boolean, otherProfile?: { name: string; avatar: string }) => {
    ...
    // Local stream is ready; attachment to the <video> element happens in
    // the useEffect that watches `state === 'active'` so the element is
    // guaranteed to be mounted (previously the active UI was not yet
    // rendered when prepareCall ran, so videos stayed blank).
    if (localStreamRef.current) {
      // nothing here — attach later
    }
  ...
  // Keep a ref of current state so the incoming-call listener (which must
  // not re-subscribe on every state change) can still decide whether we
  // are free to accept a new call.
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel('calls-realtime')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'calls', filter: `callee_id=eq.${user.id}` }, async (payload: any) => {
        ...
        if (payload.new?.status === 'ringing' && stateRef.current === 'idle') {
  ...
  // Attach local + remote streams to the video elements once the active
  // call UI is mounted. Previously prepareCall ran while state was still
  // "outgoing"/"incoming", so the <video> refs were null and nothing ever
  // appeared — and one side often had no audible audio.
  useEffect(() => {
    ...
    if (remoteEl && remoteStreamRef.current) {
      ...
      remoteEl.volume = 1;
      remoteEl.play().catch(() => {
        setTimeout(() => remoteEl.play().catch(() => {}), 300);
      });
    }
  }, [state, callId, cameraOn]);
  ...
  return (
    <>
      ...
      {/* ---------- Active call screen (once connected) — Discord-style ---------- */}
      {state === 'active' && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-[#0c0c0e] animate-fade-in">
          <div className="flex items-center justify-between px-5 py-3 border-b border-white/5 bg-black/40 backdrop-blur-md">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-500/20 ring-2 ring-emerald-500/30">
                <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
              </div>
              <div>
                <p className="text-sm font-semibold text-white leading-tight">
                  {calleeInfo?.name || callerInfo?.name || 'Call'}
                </p>
                <p className="text-xs text-white/50">{callStatus}</p>
              </div>
            </div>
            <button
              onClick={() => endCall(callId)}
              className="rounded-lg bg-rose-500/90 hover:bg-rose-500 px-3 py-1.5 text-xs font-medium text-white transition-colors"
            >
              Leave
            </button>
          </div>
          {/* Remote video fills; local PiP bottom-right */}
          <div className="relative flex-1 flex items-center justify-center overflow-hidden bg-black">
            ...
            {!remoteStreamRef.current && (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-[#111114]">
                <div className="flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-[var(--accent)] to-[var(--accent-2)] text-3xl font-bold text-white shadow-2xl">
                  {(calleeInfo?.name || callerInfo?.name || 'U').slice(0, 2).toUpperCase()}
                </div>
                <p className="text-sm text-white/60">{callStatus}</p>
              </div>
            )}
            <div className="absolute bottom-5 right-5 h-36 w-52 overflow-hidden rounded-2xl border-2 border-white/15 bg-black shadow-2xl ring-1 ring-black/50">
              <video
                ref={localVideoRef}
                autoPlay
                playsInline
              ...
              />
              {!cameraOn && (
                <div className="flex h-full flex-col items-center justify-center gap-1 bg-[#1a1a1e] text-white/50 text-xs">
                  <VideoOff size={20} />
          ...
          {/* Bottom control bar */}
          <div className="flex items-center justify-center gap-5 py-5 bg-black/50 backdrop-blur-md border-t border-white/5">
            <button
              onClick={toggleMic}
              className={`flex h-14 w-14 items-center justify-center rounded-full transition-all hover:scale-105 active:scale-95 ${
                micOn ? 'bg-white/10 text-white hover:bg-white/15' : 'bg-rose-500 text-white'
              }`}
            ...
            <button
              ...
              className={`flex h-14 w-14 items-center justify-center rounded-full transition-all hover:scale-105 active:scale-95 ${
                cameraOn ? 'bg-white/10 text-white hover:bg-white/15' : 'bg-rose-500 text-white'
              }`}
            ...
            <button
              onClick={() => endCall(callId)}
              className="flex h-14 w-14 items-center justify-center rounded-full bg-rose-500 text-white hover:bg-rose-600 transition-all hover:scale-105 active:scale-95 shadow-lg shadow-rose-500/25"
              title="End call"
            >
              <PhoneOff size={24} />
            </button>
