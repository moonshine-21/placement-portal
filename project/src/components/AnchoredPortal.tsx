...
// header. Without this, a popover that's merely `position: absolute` inside
// the header can end up VISUALLY UNDER content from whichever page is
// currently open — if that page has its own `z-50` elements (a dropdown,
// an autocomplete list, etc.) that come LATER in the DOM than the header,
// the browser's stacking rules let those elements paint on top even though
// the popover's CSS z-index looks higher on paper. That's what caused the
// profile popover to look "ghosted"/see-through behind page content: it
// wasn't actually transparent, it was being painted first and then drawn
// over.
//
// Portaling to document.body sidesteps all of that: the popover becomes a
// sibling of literally everything else on the page, so a single `z-index`
// on it is guaranteed to win against ANY other absolutely/fixed-positioned
// element on the page, no matter which component tree it came from.
//
...
// it only recalculates on real user events, so it doesn't reintroduce any
// of the compositor cost the flicker fix was designed to avoid.
// ============================================================================
