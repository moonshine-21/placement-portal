        ...
        <div className="card lg:col-span-2">
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-12 text-center">
              ...
              {matches.length === 0 ? (
                // No matches AT ALL (across every bucket). Only nudge
                // toward the profile page if there's actually nothing to
                // match against yet (no skills entered) — once skills
                // exist, "complete your profile" is the wrong message,
                // since the profile IS complete and there just isn't a
                // fitting open role right now.
                (!profile?.skills || profile.skills.length === 0) ? (
                  <>
                    <p className="text-sm text-[var(--text-muted)]">Add your skills to your profile so we can match you with open roles.</p>
                    <button onClick={() => onNavigate('profile')} className="btn-ghost btn-sm">
                      Add your skills <ArrowRight size={14} />
                    </button>
                  </>
                ) : (
                  <>
                    <p className="text-sm text-[var(--text-muted)]">Sorry, you don't match any companies with these skills right now — check back once more roles are posted.</p>
                    <button onClick={() => onNavigate('companies')} className="btn-ghost btn-sm">
                      Browse companies <ArrowRight size={14} />
                    </button>
                  </>
                )
              ) : (
                // Matches exist, just not in the currently-selected score
                // bucket (e.g. "High" is empty but "Medium"/"Low" aren't) —
                // sending them to "complete your profile" here would be
                // wrong since their profile is already what produced the
                // OTHER matches.
                <>
                      ...
                      <tr key={m.id} className="border-b border-[var(--border)] last:border-0">
                        <td className="py-3">
                          <div className="flex items-center gap-2.5 min-w-0">
                            <div
                              className="flex h-9 w-9 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl text-xs font-bold text-white aspect-square"
                              style={{ background: colorFor(c.org_name || 'CO') }}
                            >
                              {(c.org_name || 'CO').slice(0, 2).toUpperCase()}
                            </div>
                            <span className="font-medium text-[var(--text-primary)] truncate">{c.org_name}</span>
                          </div>
                        </td>
                        <td className="py-3">
          ...
          <div className="card relative overflow-hidden">
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              {matches.length > 0 && matches[0].match_score >= 85
                ...
                : profile?.skills && profile.skills.length > 0
                ? "Sorry, you don't match any companies with these skills right now — check back once more roles are posted."
                : 'Add your skills to get matched with open roles.'}
            </p>
            <button
              onClick={() => onNavigate(matches.length > 0 ? 'matches' : (profile?.skills && profile.skills.length > 0) ? 'companies' : 'profile')}
              className="btn-primary btn-sm mt-4 w-full"
            >
              {matches.length > 0 ? 'View matches' : (profile?.skills && profile.skills.length > 0) ? 'Browse companies' : 'Add your skills'} <ArrowRight size={14} />
            </button>
          </div>
