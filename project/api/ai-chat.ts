...
//   1. Gemini (Google) — tried first, if GEMINI_API_KEY (or
//      VITE_GEMINI_API_KEY) is set.
//   2. Groq — tried as a fallback if Gemini is unset, rate-limited, or
//      slow, as long as GROQ_API_KEY is set. Groq runs open models
//      (Llama) on its own inference chips rather than GPUs, so it tends
//      to answer noticeably faster than Gemini, which is useful
//      specifically for the case Gemini is having a slow/busy moment.
//      Get a free key at https://console.groq.com — no credit card
//      required. Add it to Vercel as GROQ_API_KEY and redeploy.
...
//
// Each attempt is capped at SINGLE_CALL_TIMEOUT_MS via AbortController. This
// matters a lot for the overall latency budget: without it, a single slow/
// hanging Gemini call can eat most of the edge function's execution window
// by itself, leaving no time for the retry loop to try anything else before
// Vercel kills the function — which is what makes the chat feel like it
// "hangs forever" before silently dropping to offline mode on the client.
const SINGLE_CALL_TIMEOUT_MS = 6000;
...
// even on its free tier — useful here specifically as a fast fallback for
// whenever Gemini is rate-limited or slow. Groq's API is OpenAI-compatible
  ...
  outer: for (const model of models) {
    for (let attempt = 0; attempt < maxAttemptsPerModel; attempt++) {
      if (Date.now() > deadline) {
        console.error(`[ai-chat] [${label}] overall retry deadline exceeded, giving up early`);
        break outer;
      }
      let res: Response;
      try {
        ...
        await sleep(Math.min(300 * (attempt + 1), maxBackoffMs));
        continue;
      }
...
export default async function handler(req: Request): Promise<Response> {
  ...
  // Attempt/backoff budget is deliberately kept small per provider. This
  // function runs on a time-limited Vercel Edge Function, and the browser
  // gives up on it after its own timeout too (see the `/api/ai-chat` fetch
  // in AIAssistantView.tsx) — so a generous retry budget doesn't make the
  // assistant more reliable, it just makes a genuinely busy moment take
  // far too long to finally admit defeat and fall back to quick-tips mode,
  // which is worse for the person waiting than failing over fast to the
  // next provider (or to offline mode).
  ...
  if (groqKey && Date.now() < overallDeadline) {
    const reply = await tryProvider(
