...
import { TrendingUp, Target, Building2, Award, CheckCircle2, Circle, Sparkles, ArrowRight, Calendar, Megaphone, Clock } from 'lucide-react';
import { useAuth } from '@/lib/auth';
import { supabase } from '@/lib/supabase';
import { loadCompanies, loadMatches, generateAndSaveMatches, calculateProfileCompletion } from '@/lib/data';
import type { Match, Company, Announcement, Event } from '@/lib/supabase';
...
export function DashboardView({ onNavigate }: Props) {
  ...
  const [shortlisted, setShortlisted] = useState(0); // how many of the OLDER-style `applications` this student has made (see MatchesView.tsx)
  ...
  useEffect(() => {
    (async () => {
      ...
      // Only auto-generate matches if none exist yet AND the student has
      // actually entered SOME data worth matching against (skills or
      // CGPA) — otherwise a brand new, still-empty profile would
      // immediately get a full set of low, meaningless scores the moment
      // they first land on the dashboard.
      if (!m.length && companies.length > 0 && (profile.skills?.length || profile.cgpa)) {
        m = await generateAndSaveMatches(profile, companies);
      }
      setMatches(m);

      // Count of the older `applications` table rows (used only for the
      // "Applications" stat card — the modern company_applications system
      // has its own dedicated ApplicationsView.tsx page).
      const { count } = await supabase
        .from('applications')
        .select('*', { count: 'exact', head: true })
        .eq('student_id', profile.id);
      setShortlisted(count || 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 stagger">
        ...
        <StatCard
          ...
          trend={shortlisted > 0 ? 'up' : 'flat'}
          progress={matches.length ? (shortlisted / matches.length) * 100 : 0}
        />
      </div>
                      ...
                      <tr key={m.id} className="border-b border-[var(--border)] last:border-0">
                        <td className="py-3">
                          <div className="flex items-center gap-2.5">
                            <div className="flex h-8 w-8 items-center justify-center rounded-lg text-xs font-bold text-white" style={{ background: c.logo_color }}>
                        ...
                        <td className="py-3 text-[var(--text-secondary)]">{c.package_lpa} LPA</td>
          ...
          <div className="card relative overflow-hidden">
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              {matches.length > 0 && matches[0].match_score >= 85
                ? `You're a strong match for ${matches[0].companies?.name}. Apply now to maximize your chances!`
                : matches.length > 0
