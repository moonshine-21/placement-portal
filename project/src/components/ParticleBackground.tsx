...
export function ParticleBackground() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      // Reduced particle count to cut GPU load (was a suspected flicker source)
      const count = Math.min(Math.floor((width * height) / 40000), 40);
    ...
    return () => {
      ...
      window.removeEventListener('resize', debouncedResize); if (resizeTimer) clearTimeout(resizeTimer);
    };
