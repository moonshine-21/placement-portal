...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadProfile = async (uid: string, email: string, metadata?: Record<string, unknown>) => {
    ...
    if (!data) {
      ...
      setProfile((prev) => {
        ...
        // Compare only the fields that actually affect UI / data loading.
        // JSON.stringify is fragile (key order, Date serialization) and was
        // still producing new references on background session refreshes,
        // which made every view with useEffect(..., [profile]) re-fetch
        // and flash back to skeletons — the continuous "blink" reported
        // after quizzes were added (more views started depending on profile).
        if (
          prev &&
          prev.id === next.id &&
          ...
          prev.profile_completion === next.profile_completion &&
          prev.cgpa === next.cgpa &&
  ...
  // Prevent AppShell/AppContent re-renders on every TOKEN_REFRESHED:
  // Supabase hands a new session object even when nothing meaningful changed.
  const syncSession = (newSession: Session | null) => {
    setSession((prev) => {
      // Only update React state on login/logout. Token refreshes keep the
      // same user id — ignoring them stops the whole app tree from
      // re-rendering every few minutes (the main remaining flicker source).
      const prevId = prev?.user?.id ?? null;
      const nextId = newSession?.user?.id ?? null;
      if (prevId === nextId) return prev ?? newSession;
      return newSession;
    });
  };
  ...
  useEffect(() => {
    ...
    // Supabase fires the onAuthStateChange listener below IMMEDIATELY,
    // once, the moment it's subscribed — with whatever session already
    // exists — in addition to the explicit getSession() call in Step 1.
    // That means, on every single page load, BOTH of those fire almost
    // simultaneously for the exact same login session, each independently
    // calling loadProfile(...) and then setLoading(false) once its own
    // fetch finishes. Since two separate network requests rarely finish
    // in lockstep, whichever one happened to finish FIRST could flip
    // `loading` to false a moment before the OTHER one had actually
    // finished writing the real profile into state — and the sidebar
    // (which picks its whole menu based on profile.role) would very
    // briefly render with no profile yet (defaulting to the student
    // menu), then swap to the real menu an instant later. That swap is
    // exactly the "sidebar flickers on load" bug. This ref makes sure
    // only ONE of the two duplicate events actually performs the fetch
    // for a given user — the other just syncs session/user and leaves
    // profile-loading to whichever one got there first — so there's only
    // ever one profile fetch, and therefore only one point where
    // `loading` can turn false.
    const profileFetchUidRef = { current: null as string | null };
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      if (uid) {
        if (profileFetchUidRef.current === uid) return; // the onAuthStateChange listener already claimed this fetch
        profileFetchUidRef.current = uid;
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((_event, newSession) => {
      ...
      if (uid) {
        if (profileFetchUidRef.current === uid) return; // getSession()'s Step 1 above already claimed this fetch (the very common case, since it usually resolves first)
        profileFetchUidRef.current = uid;
        ...
        profileFetchUidRef.current = null;
        setProfile(null);
        setLoading(false);
      }
    });
  ...
  // Share everything with the rest of the app.
    ...
    // refreshProfile/signOut/clearPasswordRecovery are stable enough for our purposes;
    // session/user/profile/loading are the ones that used to change identity too often.
    [session, user, profile, loading, isPasswordRecovery]
  );
