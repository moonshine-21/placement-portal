...
async function callOnce(apiKey: string, model: string, prompt: string, temperature: number, maxOutputTokens: number) {
...
export async function generateText(
  ...
  for (const model of models) {
    for (let attempt = 0; attempt < maxAttemptsPerModel; attempt++) {
      let res: Response;
      try {
        res = await callOnce(apiKey, model, prompt, temperature, maxOutputTokens);
      } catch (err) {
...
export async function generateJSON<T>(prompt: string, opts?: { temperature?: number; maxOutputTokens?: number }): Promise<T | null> {
