...
export function ProfileCardModal({ userId, onClose, onMessage, onCall }: Props) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-[420px] max-h-[90vh] overflow-y-auto scroll-thin rounded-2xl border border-white/[0.08] bg-[#1e1f22] shadow-2xl animate-fade-in-scale"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Discord-style banner — full bleed, taller, proper cover */}
        <div className="relative h-[120px] w-full overflow-hidden rounded-t-2xl bg-gradient-to-br from-[var(--accent)]/40 via-[var(--accent-2)]/30 to-[#1e1f22]">
          {profile?.banner_url ? (
            <img
          ...
          {/* Soft gradient at bottom so avatar sits cleanly */}
          <div className="absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-[#1e1f22] to-transparent" />
          <button
            onClick={onClose}
            className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-black/40 text-white/90 hover:bg-black/60 transition-colors backdrop-blur-sm"
          >
            <X size={15} />
          </button>
        </div>
        ...
        {loading ? (
          ...
          <>
            {/* Avatar overlapping banner — Discord style (circular, thick ring) */}
              ...
              <div className="relative">
                {profile.avatar_url ? (
                  <img
                    ...
                    alt=""
                    className="h-[88px] w-[88px] rounded-full object-cover ring-[6px] ring-[#1e1f22] shadow-xl"
                  />
                ) : (
                  <div className="flex h-[88px] w-[88px] items-center justify-center rounded-full bg-gradient-to-br from-[var(--accent)] to-[var(--accent-2)] text-2xl font-bold text-white ring-[6px] ring-[#1e1f22] shadow-xl">
                    {(profile.full_name || 'U').slice(0, 2).toUpperCase()}
                  </div>
                )}
                <span className="absolute bottom-1 right-1 h-4 w-4 rounded-full bg-emerald-400 ring-[3px] ring-[#1e1f22]" />
              </div>
              <div className="mt-1">
                <p className="flex items-center justify-center gap-1.5 text-[18px] font-semibold text-white tracking-tight">
                  {profile.full_name || 'Unnamed Student'}
                  {(profile.role === 'admin' || profile.role === 'owner') && <AdminBadge role={profile.role} />}
                </p>
                {profile.branch && (
                  <p className="mt-0.5 flex items-center justify-center gap-1 text-[12px] text-white/45">
