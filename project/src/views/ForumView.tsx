...
export function ForumView() {
  ...
  const sendReply = async () => {
    ...
    const { data, error } = await supabase.from('forum_replies').insert({
      ...
      author_name: profile?.full_name || profile?.email || 'User', body: replyText.trim(),
    }).select().maybeSingle();
        ...
        <div className="card">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-[var(--text-muted)] mt-1">{selectedPost.author_name} · {timeAgo(selectedPost.created_at)} · {selectedPost.views} views</p>
            </div>
