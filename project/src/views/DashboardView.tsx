...
import { loadOpenJobs, generateJobMatches, calculateProfileCompletion } from '@/lib/data';
import type { JobMatch, Announcement, Event } from '@/lib/supabase';
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.slice(0, 8).map((m) => (
                      <tr key={m.job_id} className="border-b border-[var(--border)] last:border-0">
                        <td className="py-3">
                          <div className="flex items-center gap-2.5">
                            {m.company_avatar_url ? (
                              <img src={m.company_avatar_url} alt="" className="h-8 w-8 rounded-lg object-cover" />
                            ) : (
                              <div className="flex h-8 w-8 items-center justify-center rounded-lg text-xs font-bold text-white bg-gradient-to-br from-[var(--accent)] to-[var(--accent-2)]">
                                {m.company_name.slice(0, 2).toUpperCase()}
                              </div>
                            )}
                            <span className="font-medium text-[var(--text-primary)]">{m.company_name}</span>
                          </div>
                        </td>
                        <td className="py-3">
                        ...
                        <td className="py-3 text-[var(--text-secondary)]">{m.package_lpa} LPA</td>
                        <td className="py-3 text-[var(--text-secondary)]">{m.job_name}</td>
          ...
          <div className="card relative overflow-hidden">
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              {matches.length > 0 && matches[0].match_score >= 85
                ? `You're a strong match for ${matches[0].company_name}. Apply now to maximize your chances!`
                : matches.length > 0
                ? `Focus on closing skill gaps in ${matches[0].missing_skills?.slice(0, 2).join(', ')} to boost your top match.`
                : completion >= 100
                ? 'Your profile is all set — recommendations will appear here as soon as companies post open roles.'
