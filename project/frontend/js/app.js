...
function subscribeCallStatus(callId) {
  supabase
    ...
    .on(
      ...
      (payload) => {
        ...
        if (st === "accepted") {
          hideOutgoingCall();
          joinCall(callId, true);
        } else if (st === "declined") {
          hideOutgoingCall();
          showToast("Call declined.", "info");
          activeCallId = null;
          supabase.removeChannel(supabase.channel(`call-status-${callId}`));
...
async function acceptIncomingCall() {
  ...
  try {
    ...
    if (error) throw error;
    document.getElementById("incomingCallOverlay").classList.remove("show");
    await joinCall(incomingCallId, false);
  } catch (err) {
...
const ICE_SERVERS = [
  ...
  // Free public TURN relay (Open Relay Project) — needed so calls still
  // connect when a strict firewall/NAT blocks direct peer-to-peer.
  // Both UDP and TCP variants are listed: some networks (hotels, mobile
  // carriers, corporate firewalls) block UDP relay traffic entirely, and
  // without a TCP fallback the connection just hangs on "Connecting…"
...
async function joinCall(callId, isCaller) {
  ...
  if (!overlay || !container || !localVideo || !remoteVideo) {
  ...
  try {
    ...
    rtcPeer.onconnectionstatechange = () => {
      ...
      if (rtcPeer.connectionState === "connected") {
        clearTimeout(connectTimeoutHandle);
      } else if (["failed", "disconnected", "closed"].includes(rtcPeer.connectionState)) {
    ...
    // ICE can get stuck in "checking" forever (no failure event fires) if
    // NAT traversal never succeeds — e.g. UDP relay is blocked on one side's
    // network. Without this, the UI just shows "Connecting…" indefinitely
    // with no feedback. Give it 20s, then try one ICE restart before
    // surfacing a real error to the user.
    ...
    connectTimeoutHandle = setTimeout(async () => {
      ...
      if (isCaller && !triedIceRestart) {
        ...
        showToast("Still connecting… this can take a moment on some networks.", "info");
      } else {
        showToast("Couldn't connect the call — check your network and try again.", "error");
      }
    }, 15000);
      ...
      .on("broadcast", { event: "signal" }, async ({ payload }) => {
        ...
        try {
          if (payload.kind === "offer") {
            ...
            // that race ahead of the offer (a real possibility since ICE
            // gathering can start firing before the offer message is even
            // sent) get silently dropped, which can be enough to prevent
            // the connection from ever completing.
