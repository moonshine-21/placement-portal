...
import { AdminDashboardView, AdminUsersView, AdminContentView, AdminLogsView } from '@/views/AdminViews';
...
function AppContent() {
  ...
  const role = profile?.role === 'company' ? 'company' : (profile?.role === 'admin' || profile?.role === 'owner') ? 'admin' : 'student';
  ...
  if (role !== 'admin' && adminOnlyViews.includes(currentView)) effectiveView = role === 'company' ? 'company-dashboard' : 'dashboard';
  if (role === 'admin' && ((studentOnlyViews.includes(currentView) && currentView !== 'profile') || companyOnlyViews.includes(currentView))) {
    effectiveView = 'admin-dashboard';
  }
  if (role === 'admin' && currentView === 'dashboard') effectiveView = 'admin-dashboard';
  ...
  const renderView = () => {
    switch (effectiveView) {
      ...
      case 'admin-content': return <AdminContentView />;
      case 'admin-logs': return <AdminLogsView />;
      default: return role === 'admin'
        ? <AdminDashboardView />
        : <DashboardView onNavigate={(v) => setCurrentView(v as ViewKey)} />;
    }
  };
