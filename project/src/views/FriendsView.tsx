...
export function FriendsView({ onNavigate, onOpenConversation, onStartCall }: Props) {
  ...
  const searchStudents = async (q: string) => {
    ...
    setSearching(true);
    // Trim whitespace — a trailing space typed (or pasted) before the
    // 300ms debounce fires would otherwise get sent straight into the
    // ILIKE pattern and can fail to match a name that doesn't have that
    // same trailing space stored in the database.
    const { data, error } = await supabase.rpc('search_students', { q: q.trim() });
    if (error) {
      // BUG FIX: this error used to be silently swallowed — a failed
      // search (e.g. the `search_students` database function missing or
      // out of date on the connected Supabase project) looked IDENTICAL
      // to "no students matched," with zero indication anything had gone
      // wrong. Surfacing it means a real backend problem shows up as a
      // visible error instead of a confusing empty result.
      console.error('[search_students] failed:', error);
