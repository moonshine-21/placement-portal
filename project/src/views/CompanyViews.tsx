...
export function CompanyOverviewView({ onNavigate }: { onNavigate: (view: string) => void }) {
  ...
  // Temporary diagnostic — remove after flicker is solved
  ...
  useEffect(() => {
    (async () => {
      if (!profile) return;
      // Do NOT set loading=true again after the first successful load.
      // Background profile refreshes used to blank the whole page.
  ...
  }, [profile?.id]); // depend only on id, not the whole profile object

  // Never blank the page after we have shown real content once
  if (loading && apps.length === 0) return <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[1, 2, 3, 4].map((i) => <div key={i} className="skeleton h-36 rounded-2xl" />)}</div>;
  ...
  return (
    <div className="space-y-6">
      {/* Temporary diagnostic — if this number climbs rapidly while idle, we have a re-render loop */}
      <div className="text-[10px] text-[var(--text-muted)] mb-1">renders: {renderCount.current}</div>
