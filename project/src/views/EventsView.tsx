      ...
      {showForm && (
        <form onSubmit={createEvent} className="card space-y-4 animate-slide-up">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div><label className="mb-2 block text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Type</label><select value={eventType} onChange={(e) => setEventType(e.target.value)} className="input-field">{EVENT_TYPES.map(t => <option key={t.key} value={t.key}>{t.label}</option>)}</select></div>
          </div>
