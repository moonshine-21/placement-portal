...
-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ FROM: 20260803120000_quiz_system.sql                                       │
-- └────────────────────────────────────────────────────────────────────────┘
...
-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ FROM: 20260813010000_site_settings.sql                                     │
-- └────────────────────────────────────────────────────────────────────────┘

-- Adds site-wide branding (name + logo), editable only by admin/owner
-- accounts from the admin app's "Site Settings" page, and read by
-- everyone (including logged-out visitors on the landing page).
...
-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ FROM: 20260826010000_enable_realtime_publication.sql                       │
-- └────────────────────────────────────────────────────────────────────────┘
...
/*
  ...
  See the migration file for the full explanation. Short version: a table
  only pushes postgres_changes events once it's added to the
  `supabase_realtime` publication (a separate step from RLS/policies) —
  without it, the CALLER side of a call could ring forever even after the
  callee accepts, since that transition relies on hearing the `calls` row
  flip to 'accepted' over Realtime. Safe to run repeatedly.
*/
...
-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ FROM: 20260826020000_uncap_student_search.sql                                   │
-- │ (superseded below by 20260826060000_fully_uncap_student_search.sql)             │
-- └────────────────────────────────────────────────────────────────────────┘
...
/*
  ...
  This raises the cap to 1000 as a first pass. It's still a hard
  ceiling — see the follow-up block below, which removes it entirely.
*/
...
-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ FROM: 20260826030000_allow_delete_own_messages.sql                              │
-- └────────────────────────────────────────────────────────────────────────┘
...
-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ FROM: 20260826040000_event_link.sql                                             │
-- └────────────────────────────────────────────────────────────────────────┘
...
-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ FROM: 20260826050000_allow_edit_own_messages.sql                                │
-- └────────────────────────────────────────────────────────────────────────┘
...
-- ┌────────────────────────────────────────────────────────────────────────┐
-- │ FROM: 20260826060000_fully_uncap_student_search.sql                             │
-- └────────────────────────────────────────────────────────────────────────┘

/*
  # Actually uncap friend/student search (fix the 1000-result ceiling)

  The 20260826020000 block above raised the search cap from 20 to 1000 —
  an improvement, but still a hard ceiling: on a large student body,
  anyone alphabetically past the first 1000 matches was still
  permanently unsearchable by name. This removes the LIMIT from
  search_students entirely, so the database itself no longer caps how
  many rows can come back for a search.
  ...
  including RPC calls like this one — that setting lives in your
  Supabase dashboard, not in SQL, so no migration/SQL file can change
  it. To get truly unlimited results, raise or clear that Max Rows
  setting in the dashboard too (or leave it high enough to comfortably
  exceed your real student count).
*/
