...
async function acceptIncomingCall() {
  ...
  try {
    ...
    await joinDailyCall(incomingCallId);
  } catch (err) {
...
async function joinDailyCall(callId) {
  ...
  if (!overlay || !container || typeof window.JitsiMeetExternalAPI === "undefined") {
    showToast("Call UI not available.", "error");
    return;
  }
  overlay.hidden = false;
  titleEl.textContent = "Joining call…";
  try {
    // Fetch the shared room name for this call row. No server call, no API
    // key, no billing — Jitsi's public meet.jit.si server is free to use.
    const { data: callRow, error: fetchErr } = await supabase
      .from("calls")
      .select("room_name")
      .eq("id", callId)
      .maybeSingle();
    if (fetchErr || !callRow?.room_name) throw new Error("Could not find the call room.");

    titleEl.textContent = "In call";
    container.innerHTML = "";
    activeCallFrame = new window.JitsiMeetExternalAPI("meet.jit.si", {
      roomName: callRow.room_name,
      parentNode: container,
      width: "100%",
      height: "100%",
      userInfo: { displayName: currentProfile?.full_name || "User" },
      configOverwrite: { prejoinPageEnabled: false },
      interfaceConfigOverwrite: { SHOW_JITSI_WATERMARK: false },
    });
    activeCallFrame.addListener("videoConferenceLeft", async () => {
    ...
    activeCallFrame.addListener("errorOccurred", (e) => {
      console.error("Jitsi call frame error:", e);
      showToast("Call error: " + (e?.error?.message || "connection problem"), "error");
    });
  } catch (err) {
    console.error("joinDailyCall", err);
    const reason =
      err?.message ||
      err?.errorMsg ||
      err?.error?.errorMsg ||
...
async function leaveActiveCall() {
  if (activeCallFrame) {
    try { activeCallFrame.executeCommand("hangup"); } catch (e) {}
  }
}
