...
export type Theme = 'dark' | 'light' | 'aurora' | 'midnight' | 'sunset' | 'ocean';
...
function getInitialTheme(): Theme {
  ...
  const allowed: Theme[] = ['light', 'dark', 'aurora', 'midnight', 'sunset', 'ocean'];
  if (stored && (allowed as string[]).includes(stored)) return stored as Theme;
  return 'dark';
}
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    root.classList.remove('theme-light', 'theme-dark', 'theme-aurora', 'theme-midnight', 'theme-sunset', 'theme-ocean');
