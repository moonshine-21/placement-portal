...
Answer briefly in plain language. You may use simple HTML like <strong> and <br/>. Do not invent companies the student is not matched with.`;
        ...
        if (res.ok) {
          ...
          console.warn('Gemini returned no text, falling back. Full response:', data);
        } else {
          console.warn('Gemini API error', res.status, await res.text().catch(() => ''));
        }
      } catch (err) {
        console.warn('Gemini request failed (network/CORS?):', err);
      }
    } else {
      console.info('AI Assistant: VITE_GEMINI_API_KEY is not set — using the built-in fallback assistant.');
    }

    // 3) Local profile-aware fallback
      ...
      <div className="glass flex h-[calc(100vh-8rem)] flex-col overflow-hidden">
        <div className="flex items-center gap-3 border-b border-[var(--border)] px-5 py-4">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-400">Ready · profile-aware</p>
          </div>
        </div>
