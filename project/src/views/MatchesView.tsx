...
export function MatchesView({ onNavigate }: Props) {
  ...
  if (matches.length === 0) {
    return (
      <div className="card flex flex-col items-center gap-3 py-16 text-center">
        ...
        <p className="text-sm text-[var(--text-muted)] max-w-sm">Add your skills in the Profile page to get matched with open roles, or check back once more companies post jobs.</p>
      </div>
    );
  }
            ...
            >
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl text-sm font-bold text-white flex-shrink-0" style={{ background: colorFor(c.org_name) }}>
                ...
                <button onClick={() => onNavigate('companies')} className="btn-primary btn-sm flex-shrink-0">
