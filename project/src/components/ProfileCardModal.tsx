...
export function ProfileCardModal({ userId, onClose, onMessage, onCall }: Props) {
  ...
  return (
    ...
    >
      <div
        className="modal-panel w-full max-w-[420px] max-h-[90vh] overflow-y-auto scroll-thin"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Banner + close button — same visual pattern as
            CompanyProfileCardModal. */}
        <div className="relative h-[120px] w-full overflow-hidden rounded-t-2xl bg-gradient-to-br from-[var(--accent)]/40 via-[var(--accent-2)]/30 to-[var(--bg-elevated)]">
          {profile?.banner_url ? (
            <img src={profile.banner_url} alt="" className="absolute inset-0 h-full w-full object-cover object-center" />
          ) : null}
          <div className="absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-[var(--surface-solid)] to-transparent" />
          <button onClick={onClose} className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-black/40 text-white hover:bg-black/60 backdrop-blur-sm">
            <X size={15} />
          </button>
        </div>
        ...
        {loading ? (
          ...
          <>
            <div className="-mt-12 flex flex-col items-center gap-2.5 text-center">
              ...
              {profile.avatar_url ? (
                <img src={profile.avatar_url} alt="" className="h-[88px] w-[88px] rounded-full object-cover ring-[6px] ring-[var(--surface-solid)] shadow-xl" />
              ) : (
                <div className="flex h-[88px] w-[88px] items-center justify-center rounded-full bg-gradient-to-br from-[var(--accent)] to-[var(--accent-2)] text-2xl font-bold text-white ring-[6px] ring-[var(--surface-solid)] shadow-xl">
                  {(profile.full_name || 'U').slice(0, 2).toUpperCase()}
                </div>
              )}
              <span className="absolute bottom-1 right-1 h-4 w-4 rounded-full bg-emerald-400 ring-[3px] ring-[var(--surface-solid)]" />
              </div>
              <div>
