...
function AppContent() {
  ...
  // logged in. Two cases need different loading screens:
  //
  //   1. No session yet (unknown if logged in): show a simple centered
  //      spinner — there's no sidebar to skeleton since a logged-out
  //      visitor doesn't get one.
  //
  //   2. Session exists but flags aren't loaded yet: show a SKELETON that
  //      is shaped exactly like AppShell (sidebar + header + content area)
  //      so the transition from "loading" to "real app" is just the
  //      skeleton bars being replaced by real content — no "sidebar pops
  //      in from nothing" blink. This is the key fix for the reported
  //      flicker: previously the loading screen had NO sidebar at all,
  //      then AppShell (with sidebar) appeared in one frame.
  if (loading) {
    return (
  ...
  // Session exists but feature flags aren't loaded yet — show a
  // sidebar-shaped skeleton so there's no blank→sidebar jump.
  if (session && !flagsLoaded) {
    return <AppSkeleton />;
  }
