...
export default async function handler(req, res) {
  ...
  // DAILY_DOMAIN should just be the subdomain (e.g. "placementhub"), but be
  // forgiving if someone set it to "placementhub.daily.co" or a full URL —
  // strip those so we never build a doubled-up domain like
  // "placementhub.daily.co.daily.co".
  ...
  // 1. Create the room if it doesn't exist. Daily returns 409 in some cases
  // and a 400 invalid-request-error with an "already exists" message in
  // others — both mean the room is already there, which is fine for us.
