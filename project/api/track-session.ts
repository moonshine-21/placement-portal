...
export default async function handler(req: Request, ctx: { waitUntil?: (p: Promise<unknown>) => void }): Promise<Response> {
  ...
  //
  // IMPORTANT: this logging is NOT part of the ban decision above — the
  // browser only needs `banned`/`reason`, which we already have. Doing
  // this select-then-update-or-insert sequence BEFORE responding (as this
  // used to) meant every single page load paid for two more full DB
  // round-trips purely for analytics, on top of the ban check, before
  // SessionGuard would let the rest of the app even start loading. Since
  // `SessionGuard` blocks its children (feature flags, auth, profile —
  // see sessionGuard.tsx) until this response arrives, that extra,
  // unnecessary latency was directly responsible for the multi-second
  // delay/flicker on every load. `ctx.waitUntil` (when the platform
  // provides it) keeps this work running in the background AFTER the
  // response has already been sent, so it no longer costs the visitor
  // anything. If `waitUntil` isn't available for some reason, we simply
  // don't await it — it still runs, just without a platform guarantee
  // that it'll finish if the function is torn down immediately after
  // responding, which is an acceptable trade for "never block the user."
  ...
  if (ctx?.waitUntil) {
    ctx.waitUntil(logSession);
  }

  // Tell the browser whether to show the "Access Restricted" screen —
  // sent as soon as the ban decision is known, without waiting on the
  // analytics logging above.
