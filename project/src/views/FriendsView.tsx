...
export function FriendsView({ onNavigate, onOpenConversation, onStartCall }: Props) {
  ...
  const searchStudents = async (q: string) => {
    ...
    setSearching(true);
    try {
      // Primary: RPC (SECURITY DEFINER) — works even when profile RLS is tight
      const { data, error } = await supabase.rpc('search_students', { q });
      let results: SearchResult[] = ((data as SearchResult[]) || []).filter((r) => r.id !== user?.id);

      // Fallback: direct profiles query if RPC fails or returns empty
      // (e.g. after unfriending, or if the RPC wasn't migrated yet).
      // Includes students and any non-company roles so removed friends
      // always remain findable.
      if (error || results.length === 0) {
        ...
        if (direct && direct.length > 0) {
          results = direct as SearchResult[];
        }
      }
