...
export function FriendsView({ onNavigate, onOpenConversation, onStartCall }: Props) {
  ...
  const loadAll = async () => {
    ...
    try {
      // Robust load: fetch friends rows first (no fragile embedded joins),
      // then batch-fetch the other people's profiles in one query. This
      // avoids silent join failures that previously left accepted friends
      // invisible after someone accepted a request.
      const [incomingRes, outgoingRes, accepted1Res, accepted2Res] = await Promise.all([
        supabase.from('friends').select('id, requester_id, recipient_id, status, created_at').eq('recipient_id', user.id).eq('status', 'pending'),
        supabase.from('friends').select('id, requester_id, recipient_id, status, created_at').eq('requester_id', user.id).eq('status', 'pending'),
        supabase.from('friends').select('id, requester_id, recipient_id, status, created_at').eq('recipient_id', user.id).eq('status', 'accepted'),
        supabase.from('friends').select('id, requester_id, recipient_id, status, created_at').eq('requester_id', user.id).eq('status', 'accepted'),
      ]);

      const incomingRows = (incomingRes.data || []) as any[];
      const outgoingRows = (outgoingRes.data || []) as any[];
      const acceptedRows = [...(accepted1Res.data || []), ...(accepted2Res.data || [])] as any[];

      const otherIds = new Set<string>();
      incomingRows.forEach((r) => otherIds.add(r.requester_id));
      outgoingRows.forEach((r) => otherIds.add(r.recipient_id));
      acceptedRows.forEach((r) => {
        otherIds.add(r.requester_id === user.id ? r.recipient_id : r.requester_id);
      });

      const profileMap: Record<string, { full_name: string; avatar_url: string; branch: string; bio: string }> = {};
      if (otherIds.size > 0) {
        const { data: profiles } = await supabase
          .from('profiles')
          .select('id, full_name, avatar_url, branch, bio')
      ...
      const toFriend = (r: any, otherId: string): FriendWithProfile => {
        const p = profileMap[otherId] || { full_name: 'User', avatar_url: '', branch: '', bio: '' };
        return {
          ...r,
          otherId,
          otherName: p.full_name,
          otherAvatar: p.avatar_url,
          otherBranch: p.branch,
          otherBio: p.bio,
        } as FriendWithProfile;
      };

      setIncoming(incomingRows.map((r) => toFriend(r, r.requester_id)));
      setOutgoing(outgoingRows.map((r) => toFriend(r, r.recipient_id)));

      const seen = new Set<string>();
      const acceptedList: FriendWithProfile[] = [];
      for (const r of acceptedRows) {
        if (seen.has(r.id)) continue;
        seen.add(r.id);
        const otherId = r.requester_id === user.id ? r.recipient_id : r.requester_id;
        acceptedList.push(toFriend(r, otherId));
      }
      setAccepted(acceptedList);
    } catch (err) {
      console.error('Failed to load friends:', err);
      showToast('Could not load friends list', 'error');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {accepted.map((f) => (
              <div key={f.id} className="group rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 transition-all duration-200 hover:border-[var(--accent)]/40 hover:shadow-lg hover:shadow-[var(--accent)]/5 animate-fade-in">
                <button onClick={() => setViewProfileId(f.otherId)} className="flex w-full items-start gap-3 text-left">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold truncate group-hover:text-[var(--accent)] transition-colors">{f.otherName}</p>
                ...
                <div className="mt-3 flex items-center gap-2">
                  ...
                  <button onClick={() => removeFriend(f.id, f.otherName)} className="flex h-8 w-8 items-center justify-center rounded-lg text-[var(--text-muted)] hover:text-rose-400 hover:bg-rose-500/10 transition-colors" title="Remove">
                    <X size={16} />
                  </button>
                </div>
              </div>
