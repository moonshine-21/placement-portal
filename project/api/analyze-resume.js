...
const GEMINI_MODELS = ["gemini-2.0-flash", "gemini-2.0-flash-001", "gemini-1.5-flash-latest", "gemini-1.5-flash"];
    ...
    for (const model of GEMINI_MODELS) {
      try {
        ...
        if (!upstream.ok) {
          ...
          console.error(`Gemini analyze error (${model}):`, upstream.status, errText);
          lastErr = `${model} → ${upstream.status}: ${errText.slice(0, 300)}`;
          continue;
        }
        ...
        console.error(`analyze-resume (${model}) failed:`, err);
        lastErr = `${model} → ${err.message}`;
      }
    }

    if (!parsed) {
      return res.status(200).json({ ai: false, reason: "upstream_error", detail: lastErr });
    }
