  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.reason === "upstream_error") {
        return `<strong>The AI service returned an error.</strong><br><br>Your Gemini key may be invalid, expired, or the free quota is exhausted. Check the <strong>GEMINI_API_KEY</strong> value in Vercel and that the Gemini API is enabled for your key. Meanwhile:<br><br>${ruleBasedReply(question)}`;
      }
