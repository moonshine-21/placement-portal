...
async function loadIncomingRequests() {
  ...
  try {
    const { data, error } = await supabase
      .from("friends")
      .select("id, requester_id, status, created_at, profiles!friends_requester_id_fkey1(full_name, email, avatar_url)")
...
async function loadOutgoingRequests() {
  ...
  try {
    const { data, error } = await supabase
      .from("friends")
      .select("id, recipient_id, status, profiles!friends_recipient_id_fkey1(full_name, email, avatar_url)")
...
async function loadAcceptedFriends() {
  ...
  try {
    const { data: incoming, error: e1 } = await supabase
      .from("friends")
      .select("id, requester_id, profiles!friends_requester_id_fkey1(full_name, email, avatar_url)")
      .eq("recipient_id", currentUser.id)
      .eq("status", "accepted");
    if (e1) throw e1;
    const { data: outgoing, error: e2 } = await supabase
      .from("friends")
      .select("id, recipient_id, profiles!friends_recipient_id_fkey1(full_name, email, avatar_url)")
      .eq("requester_id", currentUser.id)
      .eq("status", "accepted");
    if (e2) throw e2;
    const friends = [
