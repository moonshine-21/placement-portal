        ...
        {loading ? (
          ...
          <>
            {/* Banner image (or a gradient placeholder if none is set),
                with the close (X) button floating on top of it. */}
            <div className="relative h-[110px] w-full overflow-hidden rounded-t-2xl bg-gradient-to-br from-[var(--accent)]/40 via-[var(--accent-2)]/30 to-[var(--bg-elevated)]">
              {company.banner_url && (
                <img src={company.banner_url} alt="" className="absolute inset-0 h-full w-full object-cover object-center" />
              )}
              <div className="absolute inset-x-0 bottom-0 h-8 bg-gradient-to-t from-[var(--bg-elevated)] to-transparent" />
              <button onClick={onClose} className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-black/40 text-white hover:bg-black/60 backdrop-blur-sm transition-colors">
                <X size={15} />
              </button>
            </div>
            <div className="p-6 pt-0">
              <div className="-mt-10 mb-3 flex items-end gap-3">
                {company.avatar_url ? (
                  <img src={company.avatar_url} alt="" className="h-[72px] w-[72px] rounded-2xl object-cover ring-[5px] ring-[var(--bg-elevated)] shadow-lg" />
                ) : (
                  <div className="flex h-[72px] w-[72px] items-center justify-center rounded-2xl bg-gradient-to-br from-[var(--accent)] to-[var(--accent-2)] text-white ring-[5px] ring-[var(--bg-elevated)] shadow-lg">
                    <Building2 size={28} />
                  </div>
                )}
              </div>
