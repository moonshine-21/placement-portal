...
function AppShell({ currentView, onNavigate, onSignOut, children }: Props) {
  ...
  // Note: outside-click-to-close for each dropdown is handled by its own
  // AnchoredPortal instance below (see AnchoredPortal.tsx), not a single
  // shared document listener, since each panel now lives outside the
  // header in document.body.
  ...
  const themeOptions: { key: Theme; label: string; icon: typeof Sun }[] = [
    { key: 'dark', label: 'Cyberpunk', icon: Moon },
    { key: 'light', label: 'Minimal OS', icon: Sun },
    { key: 'aurora', label: 'Galaxy', icon: Palette },
    { key: 'midnight', label: 'Arctic', icon: Moon },
    { key: 'sunset', label: 'Ember', icon: Palette },
    { key: 'ocean', label: 'Abyss', icon: Palette },
  ];
        ...
        <header className="app-header">
          ...
          <div className="flex flex-shrink-0 items-center gap-2">
            <div className="relative">
              ...
              {/* Portal-rendered so it always sits above page content and is
                  never clipped by .app-main's overflow:hidden. */}
              <AnchoredPortal open={activeMenu === 'theme'} anchorRef={themeButtonRef} onClose={() => setActiveMenu(null)}>
                <div className="dropdown-panel w-48 p-2 animate-slide-down">
            ...
            <div className="relative">
              ...
              {/* Portal-rendered — see Step 8 of the UI rebuild: the popup
                  must always appear above all page content and never be
              ...
              <AnchoredPortal open={activeMenu === 'profile'} anchorRef={profileButtonRef} onClose={() => setActiveMenu(null)}>
                <div className="profile-popover w-[320px] overflow-hidden animate-slide-down">
