...
export function ApplicationsView() {
  ...
  const loadApps = async () => {
    ...
    if (companyErr) console.error('loadApps: failed to load applications:', companyErr);
    if (profsErr) console.error('loadApps: failed to load company profiles:', profsErr);
    ...
    const tracked: TrackedApp[] = ((companyApps as CompanyApplication[]) || []).map((a) => ({
      id: a.id,
      ...
      {apps.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {apps.map((a) => (
                <tr key={a.id} className="border-b border-[var(--border)] last:border-0 animate-fade-in">
                  <td className="py-3">
