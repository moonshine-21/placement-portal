    ...
    const touch = () => {
      try {
        ...
        if (parsed?.messages?.length > 1) {
          localStorage.setItem(
            CHAT_STORAGE_KEY,
            JSON.stringify({ ...parsed, updatedAt: Date.now() })
          );
        }
      } catch { /* */ }
    };
    const id = setInterval(touch, 60_000);
    // also touch once on mount
    touch();
      ...
      <div className="glass flex h-[calc(100vh-8rem)] flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3 border-b border-[var(--border)] px-5 py-4">
          ...
          <div>
            ...
            <p className={`text-xs ${lastSource === 'offline' ? 'text-amber-400' : 'text-emerald-400'}`}>
              {lastSource === 'gemini' || lastSource === 'backend'
                ? '⚡ AI Assistant active'
                : lastSource === 'offline'
                  ? '⚠ Quick-tips mode'
                  : 'Ready · profile-aware'}
